...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// ══════════════════════════════════════════════════════════════════════════════
    --// PESTAÑA PERMANENTE: CHAT GLOBAL v27 (UI COMO CLAUDE CHAT)
    --// ══════════════════════════════════════════════════════════════════════════════
    
    local AutoTabChat = Window:CreateTab("Chat", "rbxassetid://135141743431426")
    
    --// MÓDULO CHAT GLOBAL INTEGRADO
    local ChatGlobal = {}
    local HttpService = game:GetService("HttpService")
    local BACKEND_URL = "https://5ec3f942-cdc9-4f11-bf67-7b1276bf78bc-00-3ua8qb2prb4zp.riker.replit.dev"
    local POLL_RATE = 3
    local knownIds = {}
    
    -- Enviar mensaje (sin bloqueos)
    function ChatGlobal.Send(playerName, playerId, message)
        task.spawn(function()
    ...
    -- Conectar a backend
    function ChatGlobal.Connect(callback)
        task.spawn(function()
            while true do
                task.spawn(function()
                    ...
                    if ok then
                        ...
                        if success and data and data.messages then
                            ...
                            for i = #data.messages, 1, -1 do
                                ...
                                if msg.id and not knownIds[msg.id] then
                                    knownIds[msg.id] = true
                                    table.insert(toFire, msg)
                                end
                            end
                            for _, msg in ipairs(toFire) do
                                task.spawn(callback, msg)
                            end
                        end
                    end
                end)
    ...
    --// INPUT BAR (PARTE SUPERIOR - COMO CHAT DE CLAUDE)
    local inputBar = Instance.new("Frame")
    inputBar.Name = "InputBar"
    inputBar.Size = UDim2.new(1, -10, 0, 45)
    inputBar.Position = UDim2.new(0, 5, 0, 5)
    inputBar.BackgroundColor3 = Theme.Secondary
    inputBar.BorderSizePixel = 0
    inputBar.Parent = AutoTabChat.Page
    
    local inputBarCorner = Instance.new("UICorner")
    inputBarCorner.CornerRadius = UDim.new(0, 12)
    inputBarCorner.Parent = inputBar
    
    local inputBarStroke = Instance.new("UIStroke")
    inputBarStroke.Color = Theme.Stroke
    inputBarStroke.Thickness = 1.5
    inputBarStroke.Parent = inputBar
    
    --// BOTÓN + (IZQUIERDA)
    local addBtn = Instance.new("TextButton")
    addBtn.Name = "AddButton"
    addBtn.Size = UDim2.new(0, 35, 0, 35)
    addBtn.Position = UDim2.new(0, 5, 0.5, -17)
    addBtn.BackgroundTransparency = 1
    addBtn.Text = "+"
    addBtn.TextColor3 = Theme.Text
    addBtn.Font = Enum.Font.GothamBold
    addBtn.TextSize = 24
    addBtn.Parent = inputBar
    
    --// TEXTBOX (CENTRO)
    local messageInput = Instance.new("TextBox")
    messageInput.Name = "MessageInput"
    messageInput.Size = UDim2.new(1, -95, 0, 35)
    messageInput.Position = UDim2.new(0, 45, 0.5, -17)
    messageInput.BackgroundColor3 = Theme.Primary
    messageInput.BorderSizePixel = 0
    messageInput.TextColor3 = Theme.Text
    messageInput.Font = Enum.Font.Gotham
    messageInput.TextSize = 14
    messageInput.PlaceholderText = "Escribe un mensaje..."
    messageInput.PlaceholderColor3 = Theme.Text
    messageInput.Parent = inputBar
    
    local inputBoxCorner = Instance.new("UICorner")
    inputBoxCorner.CornerRadius = UDim.new(0, 8)
    inputBoxCorner.Parent = messageInput
    ...
    inputBoxPadding.Parent = messageInput
    
    --// BOTÓN MICRÓFONO (DERECHA - SOLO VISUAL)
    local micBtn = Instance.new("TextButton")
    micBtn.Name = "MicButton"
    micBtn.Size = UDim2.new(0, 35, 0, 35)
    micBtn.Position = UDim2.new(1, -40, 0.5, -17)
    micBtn.BackgroundTransparency = 1
    micBtn.Text = "🎤"
    micBtn.TextColor3 = Theme.Accent
    micBtn.Font = Enum.Font.GothamBold
    micBtn.TextSize = 16
    micBtn.Parent = inputBar
    
    --// CONTENEDOR DE MENSAJES (DEBAJO)
    local chatContainer = Instance.new("ScrollingFrame")
    chatContainer.Name = "ChatMessages"
    chatContainer.Size = UDim2.new(1, -10, 1, -65)
    chatContainer.Position = UDim2.new(0, 5, 0, 55)
    chatContainer.BackgroundColor3 = Theme.Primary
    chatContainer.BorderSizePixel = 0
    chatContainer.CanvasSize = UDim2.new(0, 0, 0, 0)
    chatContainer.ScrollBarThickness = 6
    chatContainer.Parent = AutoTabChat.Page
    ...
    --// CONECTAR Y MOSTRAR MENSAJES
    ChatGlobal.Connect(function(msg)
        task.spawn(function()
            if not msg or not msg.playerName then return end
            
            local msgFrame = Instance.new("Frame")
            msgFrame.Name = "Message_" .. (msg.id or "unknown")
            msgFrame.Size = UDim2.new(1, -10, 0, 40)
            msgFrame.BackgroundColor3 = Theme.Secondary
            ...
            msgLabel.Position = UDim2.new(0, 8, 0, 20)
            msgLabel.BackgroundTransparency = 1
            msgLabel.Text = msg.message or ""
            ...
            msgLabel:SetAttribute("ThemeTextRole", "Text")
            
            -- Actualizar scroll
            local newSize = chatContainer.CanvasSize.Y.Offset + 45
            chatContainer.CanvasSize = UDim2.new(0, 0, 0, newSize)
            chatContainer.CanvasPosition = Vector2.new(0, newSize)
        end)
    end)
    
    --// FUNCIÓN ENVIAR
    local function SendMessage()
        if messageInput.Text:len() > 0 then
            local player = Players.LocalPlayer
            ChatGlobal.Send(player.Name, player.UserId, messageInput.Text)
            messageInput.Text = ""
    ...
    addBtn.MouseButton1Click:Connect(SendMessage)
