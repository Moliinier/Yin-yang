...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// Enviar mensaje al backend (usando RequestAsync - sin task.spawn)
    ...
    if typeof(corner) == "function" then corner(ChatContainer, 6) end
    ...
    if typeof(corner) == "function" then corner(MessageInput, 8) end
    ...
    if typeof(corner) == "function" then corner(SendButton, 8) end
    ...
    local function RenderMessage(playerName, userId, messageText, timeStamp, isSelf)
    	...
    	if typeof(corner) == "function" then corner(MessageFrame, 8) end
