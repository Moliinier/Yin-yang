...
local Sounds = {
    ...
    CatClick = "rbxassetid://133371725828981",  -- 🐱 Sonido especial para tema Cat V1
}
...
local function playSound(soundId, volume)
    ...
    -- Si no se especifica un soundId, usar el sonido dinámico del tema actual
    local finalSoundId = soundId or CurrentClickSound
    
    if not finalSoundId or finalSoundId == "" then return end
...
local ThemePalettes = {
    ...
    --// LIGHT V1: Tema claro con fondo luminoso (85339946380507)
    LightV1 = {
        Background = Color3.fromRGB(245, 245, 248),      -- Blanco puro
        Secondary = Color3.fromRGB(230, 230, 235),       -- Gris muy claro
        AccentOff = Color3.fromRGB(200, 200, 210),       -- Gris suave
        Text = Color3.fromRGB(30, 30, 35),               -- Negro muy oscuro
        TextDim = Color3.fromRGB(100, 100, 110),         -- Gris oscuro
        Stroke = Color3.fromRGB(180, 180, 190),          -- Gris medio
        Accent = Color3.fromRGB(0, 0, 0),                -- Negro puro
        ToggleOn = Color3.fromRGB(100, 180, 255),        -- Azul claro
    },
    --// SATORU V1: Tema oscuro con efecto de poder sobrenatural (121094631325801)
    SatoruV1 = {
        Background = Color3.fromRGB(15, 12, 25),         -- Violeta oscuro
        Secondary = Color3.fromRGB(35, 25, 55),          -- Violeta medio
        AccentOff = Color3.fromRGB(60, 45, 90),          -- Violeta claro
        Text = Color3.fromRGB(240, 235, 255),            -- Blanco con tinte violeta
        TextDim = Color3.fromRGB(170, 150, 200),         -- Violeta pálido
        Stroke = Color3.fromRGB(150, 100, 255),          -- Violeta brillante
        Accent = Color3.fromRGB(200, 150, 255),          -- Violeta magenta
        ToggleOn = Color3.fromRGB(200, 150, 255),        -- Violeta magenta
    },
    --// PURPLE ANIMATED: Tema morado con animación de brillo rotativo (99628709047127)
    PurpleAnimated = {
        Background = Color3.fromRGB(20, 10, 40),         -- Morado profundo
        Secondary = Color3.fromRGB(40, 20, 70),          -- Morado medio
        AccentOff = Color3.fromRGB(70, 40, 110),         -- Morado claro
        Text = Color3.fromRGB(240, 230, 255),            -- Blanco con tinte morado
        TextDim = Color3.fromRGB(180, 150, 220),         -- Morado pálido
        Stroke = Color3.fromRGB(150, 80, 220),           -- Morado brillante
        Accent = Color3.fromRGB(200, 100, 255),          -- Morado magenta
        ToggleOn = Color3.fromRGB(200, 100, 255),        -- Morado magenta
    },
}
...
local ThemeBackgroundImages = {
    ...
    LightV1 = "rbxassetid://85339946380507",  -- ✨ Fondo luminoso
    SatoruV1 = "rbxassetid://121094631325801",  -- ⚡ Satoru con poder
    PurpleAnimated = "rbxassetid://99628709047127",  -- 🌀 Brillo morado animado
}
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:SetTheme(themeName)
        ...
        --// 🐱 SISTEMA DE SONIDO Y EFECTO DINÁMICO POR TEMA
        if themeName == "CatV1" then
            -- Tema especial del gato: Sonido de gato + Efecto rainbow rosa-blanco rápido
            CurrentClickSound = Sounds.CatClick
            self:SetTextEffect("CatRainbow")
            print("🐱 Tema Cat V1 activado - Sonido de gato especial + Efecto rainbow rosa-blanco")
        elseif themeName == "PurpleAnimated" then
            -- Tema morado animado: Rotación continua de la imagen con efecto de brillo
            ...
            if BackgroundArt then
                ...
                animationConnection = RunService.Heartbeat:Connect(function(dt)
                    if BackgroundArt and BackgroundArt.Parent then
                        BackgroundArt.Rotation = (BackgroundArt.Rotation + 1) % 360
                    end
                end)
            end
            print("🌀 Tema Purple Animated activado - Rotación continua con brillo")
        elseif themeName == "SatoruV1" then
            -- Tema Satoru: Pulsación suave de la imagen como si fuera poder sobrenatural
            CurrentClickSound = Sounds.Click
            if BackgroundArt then
                if animationConnection then
                    animationConnection:Disconnect()
                ...
                animationConnection = RunService.Heartbeat:Connect(function(dt)
                    elapsed = elapsed + dt
                    if BackgroundArt and BackgroundArt.Parent then
                        local pulse = 1 + 0.15 * math.sin(elapsed * 3)
                        BackgroundArt.ImageScaleType = Enum.ScaleType.Fit
                        BackgroundArt.Size = UDim2.new(pulse, 0, pulse, 0)
                    end
                end)
            end
            print("⚡ Tema Satoru V1 activado - Efecto de poder pulsante")
        elseif themeName == "LightV1" then
            -- Tema Light V1: Sin animación, solo ambiente brillante
            CurrentClickSound = Sounds.Click
            if animationConnection then
                animationConnection:Disconnect()
                animationConnection = nil
            end
            print("✨ Tema Light V1 activado - Ambiente luminoso")
        else
            -- Otros temas: Sonido de click normal + Efecto normal
            CurrentClickSound = Sounds.Click
    ...
    local animationConnection = nil  -- Para animaciones de imágenes de tema
...
if DEMO_ACTIVO then
    ...
    local DemoUI = _G.YinYang:CreateWindow("Yin Yang - DEMO v24", "Dark")
    ...
    local temas = {
        ...
        "CatV1",  -- 🐱 NUEVO: Tema del gato
        "LightV1",  -- ✨ NUEVO: Tema claro
        "SatoruV1",  -- ⚡ NUEVO: Tema de Satoru
        "PurpleAnimated"  -- 🌀 NUEVO: Tema morado animado
    }
