...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// Enviar mensaje al backend (manteniendo RequestAsync con task.spawn para no bloquear)
    local function BackendSendMessage(playerName, playerId, message)
        task.spawn(function()
            ...
            if not ok or not response then
                warn("[ChatGlobal] Error al enviar mensaje al backend:", response)
                return
            end

            if response.Success ~= true then
                warn("[ChatGlobal] Backend respondió con error:", response.StatusCode, response.StatusMessage)
            end
        end)
    end

    --// Helpers seguros para el chat (DEBEN IR ANTES de StartBackendPolling)
    local function SafeChatTimestamp(ts)
        if typeof(ts) == "number" then
            return os.date("%H:%M:%S", ts)
        end

        if typeof(ts) == "string" then
            local asNumber = tonumber(ts)
            if asNumber then
                return os.date("%H:%M:%S", asNumber)
            end

            local h, m, s = ts:match("^(%d%d):(%d%d):(%d%d)$")
    ...
    local function MakeServerMsgKey(msg)
        return tostring(
            msg.id
            or (tostring(msg.playerId or msg.playerUserId or "0")
                .. "|"
                .. tostring(msg.timestamp or "")
                .. "|"
                .. tostring(msg.message or ""))
        )
    end

    --// Polling del backend: mensajes + typing
        ...
        task.spawn(function()
            while true do
                ...
                if ok and response and response.Success and response.Body then
                    local decodedOk, data = pcall(HttpService.JSONDecode, HttpService, response.Body)

                    if decodedOk and data and type(data.messages) == "table" then
                        ...
                        for _, msg in ipairs(data.messages) do
                            local key = MakeServerMsgKey(msg)

                            if not knownServerIds[key] then
                                knownServerIds[key] = true

                                local eventType = tostring(msg.type or msg.event or ""):lower()

                                if eventType == "typing" then
                                    if Window.ShowTypingIndicator then
                                        Window.ShowTypingIndicator(
                                            msg.playerName or "Unknown",
                                            msg.playerId or msg.playerUserId or 0,
                                            5
                                        )
                                    end
                                else
                                    if tostring(msg.playerId) ~= tostring(LocalPlayer.UserId) then
                                        task.spawn(function()
                                            local safeOk, err = pcall(onNewMessage, msg)
                                            if not safeOk then
                                                warn("[ChatGlobal] Error renderizando mensaje entrante:", err)
                                            end
                                        end)
                                    end
                                end
    ...
    local function RenderMessage(playerName, userId, messageText, timeStamp, isSelf)
    	-- Color adaptable según si es mensaje propio o de otro
    	local backgroundColor = isSelf and Theme.Accent or Theme.Secondary
    	local backgroundTransparency = isSelf and 0.15 or 0.25
    	local textColor = isSelf and Color3.fromRGB(255, 255, 255) or Theme.Text
    	local nameColor = isSelf and Color3.fromRGB(200, 255, 200) or Theme.Accent
    	...
    	MessageFrame:SetAttribute("ThemeRole", isSelf and "Accent" or "Secondary")
    	corner(MessageFrame, 8)
    ...
    local TypingIndicators = TypingIndicators or {}

    local function ShowTypingIndicator(playerName, userId, duration)
        duration = duration or 5
        userId = tostring(userId or 0)
        playerName = tostring(playerName or "Unknown")

        local existing = TypingIndicators[userId]
        if existing then
            existing.ExpiresAt = os.clock() + duration
            if existing.Frame and existing.Frame.Parent then
                local nameLabel = existing.Frame:FindFirstChild("NameLabel")
                if nameLabel and nameLabel:IsA("TextLabel") then
                    nameLabel.Text = playerName .. " está escribiendo..."
                end
            end
            return
        end
        ...
        TypingFrame:SetAttribute("ThemeRole", "Secondary")
        if typeof(corner) == "function" then corner(TypingFrame, 8) end
        ...
        if typeof(corner) == "function" then corner(AvatarLabel, 16) end
        ...
        local token = tostring(os.clock()) .. "_" .. userId
        ...
        task.spawn(function()
            ...
            while TypingIndicators[userId]
                and TypingIndicators[userId].Token == token
                and TypingFrame
                and TypingFrame.Parent do

                i = (i % #dotFrames) + 1
                DotsLabel.Text = dotFrames[i]
                task.wait(0.45)

                if os.clock() >= TypingIndicators[userId].ExpiresAt then
                    break
                end
            end

            local current = TypingIndicators[userId]
    ...
    StartBackendPolling(function(msg)
        local eventType = tostring(msg.type or msg.event or ""):lower()
        if eventType == "typing" then
            ShowTypingIndicator(msg.playerName or "Unknown", msg.playerId or msg.playerUserId or 0, 5)
            return
        end

        local stamp = SafeChatTimestamp(msg.timestamp)
        AddChatMessage(msg.playerName, msg.playerId, msg.message, stamp)
        RenderMessage(msg.playerName, msg.playerId, msg.message, stamp, false)
    end)
    ...
    local function StartHeartbeat()
    	task.spawn(function()
    		while true do
    			...
    			if not ok then
    				-- Error en la request, pero continuamos
    			elseif not response.Success then
    				-- Backend respondió con error, pero continuamos
    			end
    		end
    	end)
    end
