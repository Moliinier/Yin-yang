...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    local LibrarySizePresets = {
        ...
        Normal = { Width = 420, Height = 340 },
        Large  = { Width = 560, Height = 680 },
    }

    local LibrarySizeMode = LibrarySizePresets[SavedConfig.LibrarySizeMode] and SavedConfig.LibrarySizeMode or "Normal"

    local function getCurrentLibraryPreset()
        return LibrarySizePresets[LibrarySizeMode] or LibrarySizePresets.Normal
    end
    ...
    function Window:SetLibraryVersion(mode)
        if mode ~= "Small" and mode ~= "Normal" and mode ~= "Large" then
            mode = "Normal"
        end

        LibrarySizeMode = mode
        self.LibrarySizeMode = mode
        self.LibraryHeight = getCurrentLibraryPreset().Height
        SavedConfig.LibrarySizeMode = mode
    ...
    function Window:CreateTab(name, iconAsset)
        ...
        function Tab:CreateToggle(text, default, callback)
            ...
            local function applyState(newState, silent)
                state = newState
            ...
            return {
                ...
                Set = function(_, value, silent)
                    applyState(value and true or false, silent)
                end
            }
        end
    ...
    --// ════════════════════════════════════════════════════════════════
    AutoTabAjustes:CreateLabel("Tamaño de la Librería", 12)
    AutoTabAjustes:CreateDivider()
    
    local sizeToggles = {}
    local applyingSize = false

    local function applyExclusiveSize(mode)
        print("applyExclusiveSize()")
        print("mode =", mode)
        print("Window:SetLibraryVersion =", Window.SetLibraryVersion)

        if applyingSize then return end
        applyingSize = true

        Window:SetLibraryVersion(mode)

        print("Calling Small:Set")
        sizeToggles.Small:Set(mode == "Small", true)

        print("Calling Normal:Set")
        sizeToggles.Normal:Set(mode == "Normal", true)

        print("Calling Large:Set")
        sizeToggles.Large:Set(mode == "Large", true)

        applyingSize = false
    end

    sizeToggles.Small = AutoTabAjustes:CreateToggle("Más Pequeño", SavedConfig.LibrarySizeMode == "Small", function(state)
        if state then applyExclusiveSize("Small") end
    end)

    sizeToggles.Normal = AutoTabAjustes:CreateToggle("Normal (Original)", SavedConfig.LibrarySizeMode == "Normal" or SavedConfig.LibrarySizeMode == nil, function(state)
        if state then applyExclusiveSize("Normal") end
    end)

    sizeToggles.Large = AutoTabAjustes:CreateToggle("Más Grande", SavedConfig.LibrarySizeMode == "Large", function(state)
        if state then applyExclusiveSize("Large") end
    end)

    print("===== SIZE DEBUG =====")
    print("Window.LibrarySizeMode:", Window.LibrarySizeMode)
    print("sizeToggles:", sizeToggles)
    print("sizeToggles.Small:", sizeToggles.Small)
    print("sizeToggles.Normal:", sizeToggles.Normal)
    print("sizeToggles.Large:", sizeToggles.Large)

    if sizeToggles.Small then
        print("Small.Set:", sizeToggles.Small.Set)
    end

    if sizeToggles.Normal then
        print("Normal.Set:", sizeToggles.Normal.Set)
    end

    if sizeToggles.Large then
        print("Large.Set:", sizeToggles.Large.Set)
    end

    print("About to call applyExclusiveSize...")
    applyExclusiveSize(Window.LibrarySizeMode)
