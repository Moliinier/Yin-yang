...
local ThemePalettes = {
    ...
    Pink = {
        ...
        Stroke = Color3.fromRGB(220, 100, 170),
        Accent = Color3.fromRGB(255, 120, 200),
        ToggleOn = Color3.fromRGB(255, 120, 200),
    },
}
...
function ZeroUI:CreateWindow(title_text, startTheme)
    ...
    function Window:CreateTab(name, iconAsset)
        ...
        function Tab:CreateServerInfoCard()
            ...
            local _, maxVal = createStatTile(Grid, "⚙️ Máximo")
            local _, pingVal = createStatTile(Grid, "📡 Latencia")
            local _, idVal = createStatTile(Grid, "🆔 ID Servidor")
            local _, fpsVal = createStatTile(Grid, "⚡ FPS")
            local _, timeVal = createStatTile(Grid, "⏱️ Tiempo")

            idVal.Text = (game.JobId ~= "" and string.sub(game.JobId, 1, 8)) or "Studio"
            playersVal.Text = "..."
            ...
            task.spawn(function()
                while Card.Parent do
                    ...
                    local okfps, fps = pcall(function()
                        return math.floor(1 / game:GetService("RunService").RenderStepped:Wait())
                    end)
                    fpsVal.Text = (okfps and fps) and (tostring(fps) .. " FPS") or "N/A"
