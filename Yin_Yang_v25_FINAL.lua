...
local TextFonts = {
    Normal = Enum.Font.GothamMedium,      -- Fuente estándar
    Bold = Enum.Font.GothamBold,          -- Fuente en negrita
    Code = Enum.Font.Monospace,           -- Fuente monoespaciada (código)
    Elegant = Enum.Font.BuildersFont,     -- Fuente elegante
    Titulo = Enum.Font.Fantasy,           -- Fuente de fantasía para títulos
    Arcade = Enum.Font.GothamBlack,       -- Fuente negra potente
}

--// Variable para font actual
local CurrentFont = TextFonts.Normal
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:SetTextEffect(mode)
        ...
        elseif mode == "RainbowDarkWhite" then
            -- 🌈 NUEVO: Efecto Rainbow que cambia LENTAMENTE de NEGRO a BLANCO
            -- Transición suave: Negro (0,0,0) → Gris → Blanco (255,255,255)
            textEffectConnection = track(RunService.Heartbeat:Connect(function(dt)
                elapsed = elapsed + dt
                -- Velocidad muy lenta: 0.3 = cambia lentamente, sube a 1 para más rápido
                local progress = (elapsed * 0.3) % 1
                
                -- Interpolación suave de Negro a Blanco
                local greyValue = math.floor(progress * 255)
                applyTextColorToAll(Color3.fromRGB(greyValue, greyValue, greyValue))
            end))
        else
...
print("✅ Yin Yang v25 - FINAL EDITION 🎉")
print("✨ CAMBIOS v25:")
print("  ✓ Orange duplicado eliminado (mantuvimos NaranjaV1)")
print("  ✓ Sonido CatClick activado automáticamente en tema Cat V1 (133371725828981)")
print("  ✓ 6 Fuentes de texto personalizadas agregadas")
print("  ✓ Efecto Rainbow Dark-White añadido (Negro → Blanco lentamente)")
print("  ✓ ¡Librería cargada y lista para usar!")
...
if DEMO_ACTIVO then
    ...
    -- =========================================================
    TabTemas:CreateDivider()
    TabTemas:CreateLabel("🎨 Fuentes de Texto Personalizadas", 14)
    TabTemas:CreateDivider()

    TabTemas:CreateButton("📝 Normal (Gotham)", function()
        CurrentFont = TextFonts.Normal
        print("✓ Fuente: Normal (Gotham Medium)")
    end)

    TabTemas:CreateButton("💪 Bold (Gotham)", function()
        CurrentFont = TextFonts.Bold
        print("✓ Fuente: Bold (Gotham Bold)")
    end)

    TabTemas:CreateButton("💻 Código (Monospace)", function()
        CurrentFont = TextFonts.Code
        print("✓ Fuente: Monospace (para código)")
    end)

    TabTemas:CreateButton("✨ Elegante (Builders)", function()
        CurrentFont = TextFonts.Elegant
        print("✓ Fuente: Elegante y sofisticada")
    end)

    TabTemas:CreateButton("🎭 Fantasía (Fantasy)", function()
        CurrentFont = TextFonts.Titulo
        print("✓ Fuente: Fantasía para títulos épicos")
    end)

    TabTemas:CreateButton("⚫ Arcade (Gotham Black)", function()
        CurrentFont = TextFonts.Arcade
        print("✓ Fuente: Arcade potente y fuerte")
    end)
