...
--// CONTADOR DE JUGADORES EN TIEMPO REAL (solo en el servidor actual)
local PlayerCountPill = mk("Frame", {
...
corner(PlayerCountPill, 999)

local playerCountStroke = mk("UIStroke", {
    ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
    Thickness = 1,
    Transparency = 0.35,
    Color = Theme.Stroke,
...
playerCountStroke:SetAttribute("ThemeRole", "Stroke")

local PlayerCountLabel = mk("TextLabel", {
...
PlayerCountLabel:SetAttribute("ThemeTextRole", "Text")

local function updatePlayerCount()
    local count = #Players:GetPlayers()
    PlayerCountLabel.Text = "Jug. " .. tostring(count)
end

updatePlayerCount()
track(Players.PlayerAdded:Connect(updatePlayerCount))
track(Players.PlayerRemoving:Connect(updatePlayerCount))
...
function Tab:CreateSlider(text, min, max, default, callback)
    ...
    local function formatValue(v)
        local rounded = math.floor((v * precision) + 0.5) / precision
        if math.abs(rounded - math.floor(rounded)) < 1e-9 then
    ...
    local ValueBox = mk("TextBox", {
        ...
        PlaceholderText = formatValue(value),
        TextColor3 = Theme.AccentText or Theme.Accent,
        Font = Enum.Font.GothamBold,
        TextSize = 12,
        ClearTextOnFocus = false,
        TextXAlignment = Enum.TextXAlignment.Center,
    ...
    ValueBox:SetAttribute("ThemeRole", "Background")
    ValueBox:SetAttribute("ThemeTextRole", "Accent")
    corner(ValueBox, 6)
    stroke(ValueBox, Theme.Stroke, 1, 0.7)
    ...
    corner(SliderThumb, 999)
    stroke(SliderThumb, Theme.Stroke, 1, 0.45)

    local thumbGlow = mk("UIGradient", {
        Color = ColorSequence.new({
    ...
    local function applyValue(newValue, fireCallback)
        ...
        value = math.floor((clamped * precision) + 0.5) / precision
        local denom = (max - min)
        local pct = 0
        if denom ~= 0 then
            pct = math.clamp((value - min) / denom, 0, 1)
        end
        SliderFill.Size = UDim2.new(pct, 0, 1, 0)
        SliderThumb.Position = UDim2.new(0, 12 + math.floor((SliderBackground.AbsoluteSize.X - 20) * pct + 0.5), 0, 42)
        local display = formatValue(value)
        ValueBox.Text = display
        ValueBox.PlaceholderText = display
    ...
    local function setFromPointer(screenX, fireCallback)
        local left = SliderBackground.AbsolutePosition.X
        local width = SliderBackground.AbsoluteSize.X
        if width <= 0 then return end
        local pct = math.clamp((screenX - left) / width, 0, 1)
        applyValue(min + (max - min) * pct, fireCallback)
    end

    local function beginDrag(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            isDragging = true
            setFromPointer(input.Position.X, true)
        end
    end

    SliderBackground.Active = true
    SliderThumb.Active = true
    SliderBackground.InputBegan:Connect(beginDrag)
    ...
    ValueBox.FocusLost:Connect(function(enterPressed)
        local typed = tonumber((ValueBox.Text or ""):gsub(",", "."))
        if typed then
            applyValue(typed, true)
        else
            ValueBox.Text = formatValue(value)
            ValueBox.PlaceholderText = formatValue(value)
        end
    end)
    ...
    return {
        Set = function(newValue)
            applyValue(tonumber(newValue) or value, true)
        end,
        Get = function()
            return value
        end,
...
function Tab:CreateInputField(labelText, placeholderText, defaultText, callback)
    ...
    local Box = mk("TextBox", {
        ...
        BorderSizePixel = 0,
        Text = currentText,
        PlaceholderText = placeholderText or labelText,
        TextColor3 = Theme.Text,
    ...
    corner(Box, 6)
    stroke(Box, Theme.Stroke, 1, 0.75)

    local function commit()
        currentText = Box.Text or ""
        if callback then
    ...
    return {
        Set = function(newValue)
            currentText = tostring(newValue or "")
