...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    -- Efecto Off
    AutoTabEfectos:CreateButton("Off", function()
    ...
    --// ══════════════════════════════════════════════════════════════════════════════
    --// PESTAÑA PERMANENTE: CHAT GLOBAL v28
    --// ══════════════════════════════════════════════════════════════════════════════
    
    local AutoTabChatGlobal = Window:CreateTab("Chat", "rbxassetid://135141743431426")
    
    -- Badge "New" que cambia amarillo ↔ blanco (pixelado)
    task.spawn(function()
        local chatTabLabel = AutoTabChatGlobal.Parent:FindFirstChild("TextLabel")
        if chatTabLabel then
            local newBadge = Instance.new("TextLabel")
            newBadge.Name = "NewBadge"
            newBadge.Size = UDim2.new(0, 30, 0, 16)
            newBadge.Position = UDim2.new(1, -35, 1, 5)
            newBadge.BackgroundTransparency = 1
            newBadge.Text = "New"
            newBadge.TextSize = 10
            newBadge.Font = Enum.Font.GothamBold
            newBadge.Parent = AutoTabChatGlobal.Parent
            
            -- Efecto rainbow amarillo ↔ blanco pixelado
            task.spawn(function()
                local colorIndex = 1
                local colors = {
                    Color3.fromRGB(255, 255, 0),    -- Amarillo
                ...
                while newBadge and newBadge.Parent do
                    newBadge.TextColor3 = colors[colorIndex]
                    colorIndex = colorIndex == 1 and 2 or 1
                    task.wait(0.15) -- Cambio rápido = efecto pixelado
                end
            end)
        end
    end)
    ...
    AutoTabChatGlobal:CreateLabel("CHAT GLOBAL EN TIEMPO REAL", 14)
    AutoTabChatGlobal:CreateDivider()
    
    local chatContainer = Instance.new("ScrollingFrame")
    chatContainer.Name = "ChatContainer"
    chatContainer.Size = UDim2.new(1, -10, 1, -80)
    chatContainer.Position = UDim2.new(0, 5, 0, 50)
    chatContainer.BackgroundColor3 = Color3.fromRGB(25, 25, 30)
    chatContainer.BorderSizePixel = 0
    chatContainer.CanvasSize = UDim2.new(0, 0, 0, 0)
    chatContainer.ScrollBarThickness = 8
    chatContainer.Parent = AutoTabChatGlobal.Parent
    
    local corner = Instance.new("UICorner")
    corner.CornerRadius = UDim.new(0, 8)
    corner.Parent = chatContainer
    
    -- Conectar al módulo de Chat Global
    -- Cargar módulo de Chat Global de forma segura (ver línea siguiente)
    -- Cargar módulo de Chat Global de forma segura
    local ChatGlobal = {}
    local moduleLoaded = false
    
    local ok, result = pcall(function()
        ChatGlobal = require(script:WaitForChild("YinYang_GlobalChat"))
        moduleLoaded = true
    end)
    
    if not moduleLoaded then
        warn("[Chat Global] Módulo no encontrado. El chat funcionará sin backend.")
        -- Crear función stub para que no cause error
        ChatGlobal.Connect = function(callback) end
        ChatGlobal.Send = function(name, id, msg) end
    end
    ...
    ChatGlobal.Connect(function(msg)
        ...
        messageFrame.Name = "Message_" .. msg.id
        messageFrame.Size = UDim2.new(1, -10, 0, 60)
        messageFrame.BackgroundColor3 = Color3.fromRGB(35, 35, 40)
        ...
        infoLabel.Position = UDim2.new(0, 50, 0, 5)
        infoLabel.BackgroundTransparency = 1
        infoLabel.Text = msg.playerName .. "  " .. os.date("%H:%M", msg.timestamp)
        infoLabel.TextColor3 = Color3.fromRGB(200, 200, 200)
        infoLabel.TextSize = 12
        infoLabel.Font = Enum.Font.GothamBold
        infoLabel.TextXAlignment = Enum.TextXAlignment.Left
        infoLabel.Parent = messageFrame
        ...
        local canvasSize = chatContainer.CanvasSize
        chatContainer.CanvasSize = UDim2.new(0, 0, 0, canvasSize.Y.Offset + 70)
        chatContainer.CanvasPosition = Vector2.new(0, chatContainer.CanvasSize.Y.Offset)
    end)
    
    AutoTabChatGlobal:CreateDivider()
    AutoTabChatGlobal:CreateLabel("FUNCIÓN: Muestra mensajes en tiempo real de todos los jugadores", 10)
    ...
    inputContainer.Parent = AutoTabChatGlobal.Parent
    
    local inputCorner = Instance.new("UICorner")
    inputCorner.CornerRadius = UDim.new(0, 6)
    inputCorner.Parent = inputContainer
    
    -- TextBox para escribir
    local messageInput = Instance.new("TextBox")
    messageInput.Name = "MessageInput"
    messageInput.Size = UDim2.new(1, -50, 1, 0)
    messageInput.Position = UDim2.new(0, 5, 0, 0)
    messageInput.BackgroundColor3 = Color3.fromRGB(45, 45, 50)
    ...
    messageInput.PlaceholderText = "Escribe tu mensaje..."
    ...
    sendButton.Position = UDim2.new(1, -40, 0, 0)
    sendButton.BackgroundColor3 = Color3.fromRGB(100, 150, 255)
...
local DEMO_ACTIVO = false  -- ✅ DESACTIVADA - La librería crea las pestañas automáticamente
