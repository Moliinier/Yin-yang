...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:CreateTab(nameSpanish, nameEnglishOrIcon, iconAsset)
        ...
        function Tab:CreateSlider(textSpanish, textEnglishOrMin, minOrMax, maxOrDefault, defaultOrCallback, callbackMaybe)
            --// COMPATIBILIDAD:
            --// 1) Antiguo: CreateSlider(text, min, max, default, callback)
            --// 2) Nuevo bilingüe: CreateSlider(textSpanish, textEnglish, min, max, default, callback)
            local textEnglish = textSpanish
            local min, max, default, callback

            if type(textEnglishOrMin) == "string" then
                textEnglish = textEnglishOrMin
                min = minOrMax
                max = maxOrDefault
                default = defaultOrCallback
                callback = callbackMaybe
            else
                min = textEnglishOrMin
                max = minOrMax
