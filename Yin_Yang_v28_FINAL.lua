...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    mk("Frame", {Size = UDim2.new(0, 1, 1, 0), Position = UDim2.new(1, -1, 0, 0), BackgroundColor3 = Theme.Stroke, BackgroundTransparency = 0.82, BorderSizePixel = 0, ZIndex = 8}, TabList):SetAttribute("ThemeRole", "Stroke")
    ...
    --// TAMAÑO DE LA VENTANA: modo pequeño/grande + altura personalizable
    local LibrarySizePresets = {
        Small = { Width = 420, DefaultHeight = 340, MaxHeight = 340 },
        Large = { Width = 560, DefaultHeight = 680, MaxHeight = 800 },
    }

    local LibrarySizeMode = ((SavedConfig.LibrarySizeMode or "Small") == "Large") and "Large" or "Small"
    local LibraryHeight = tonumber(SavedConfig.LibraryHeight) or LibrarySizePresets[LibrarySizeMode].DefaultHeight
    shownSize = UDim2.new(0, LibrarySizePresets[LibrarySizeMode].Width, 0, LibraryHeight)

    local function getCurrentLibraryPreset()
        return LibrarySizePresets[LibrarySizeMode] or LibrarySizePresets.Small
    end

    local function clampLibraryHeight(height)
        local preset = getCurrentLibraryPreset()
        local numericHeight = tonumber(height) or preset.DefaultHeight
        return math.clamp(math.floor(numericHeight + 0.5), 320, preset.MaxHeight)
    end
    ...
    local function updateWindowSize()
        ...
        local preset = getCurrentLibraryPreset()
        local width = math.min(preset.Width, math.floor(screen.X * 0.92))
        local height = math.min(clampLibraryHeight(LibraryHeight), math.floor(screen.Y * 0.92))
    ...
    function Window:SetLibraryVersion(isLarge)
        ...
        local preset = getCurrentLibraryPreset()
        LibraryHeight = preset.DefaultHeight
        self.LibrarySizeMode = LibrarySizeMode
        self.LibraryHeight = LibraryHeight
        SavedConfig.LibrarySizeMode = LibrarySizeMode
        SavedConfig.LibraryHeight = LibraryHeight
        SaveConfig()
        updateWindowSize()
    end

    function Window:SetLibraryHeight(height)
        LibraryHeight = clampLibraryHeight(height)
        self.LibraryHeight = LibraryHeight
        SavedConfig.LibraryHeight = LibraryHeight
        SaveConfig()
        updateWindowSize()
    ...
    --// ════════════════════════════════════════════════════════════════
    AutoTabAjustes:CreateToggle("Usar versión grande de librería", Window.LibrarySizeMode == "Large", function(state)
        Window:SetLibraryVersion(state)
        AutoTabAjustes:CreateLabel(state and "Versión grande activa" or "Versión pequeña activa", 11)
    end)

    AutoTabAjustes:CreateSlider("Altura de la librería", 320, 800, Window.LibraryHeight, function(value)
        Window:SetLibraryHeight(math.floor(value + 0.5))
    end)
    ...
    local function RenderMessage(playerName, userId, messageText, timeStamp, isSelf)
    	local MessageFrame = mk("Frame", {
    		...
    		BackgroundColor3 = isSelf and Theme.Accent or Theme.Secondary,
    		BackgroundTransparency = 0.3,
    		BorderSizePixel = 0,
    		LayoutOrder = #ChatMessages,
    		ZIndex = 12,
    	})
    	corner(MessageFrame, 6)
    	...
    	local InfoLabel = mk("TextLabel", {
    		...
    		BackgroundTransparency = 1,
    		Text = tostring(playerName or "Unknown") .. " • " .. tostring(timeStamp or os.date("%H:%M:%S")),
    		Font = Enum.Font.GothamBold,
    		TextSize = 11,
    		TextColor3 = Theme.Text,
    		TextXAlignment = Enum.TextXAlignment.Left,
