...
--// 💾 SISTEMA DE GUARDADO/PERSISTENCIA (v26)
...
local function SaveConfig()
    ...
    local jsonStr = game:GetService("HttpService"):JSONEncode(configData)
    pcall(function()
        writefile(ConfigFile, jsonStr)
        print("✅ Configuración guardada: " .. CurrentTheme)
    end)
end
...
local function LoadConfig()
    pcall(function()
        ...
        if jsonStr then
            ...
            SavedConfig = config
            print("✅ Configuración cargada: " .. (config.theme or "Default"))
            return config
        end
    end)
    return nil
...
local function playSound(soundId, volume)
    ...
    -- 🎵 v26: Usar sonido personalizado del tema si no se especifica uno
    local finalSoundId = soundId
    
    if not finalSoundId and CurrentTheme and ThemeClickSounds[CurrentTheme] then
        finalSoundId = ThemeClickSounds[CurrentTheme]
    end
    
    if not finalSoundId or finalSoundId == "" then 
        finalSoundId = Sounds.Click  -- Fallback
    end
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// 🌙 SISTEMA DE TÍTULO ANIMADO CON RAINBOW (v26)
    local TitleAnimIndex = 0
    local TitleColorIndex = 0
    local TitleAnimConnection
    
    local function StartTitleAnimation()
        if TitleAnimConnection then TitleAnimConnection:Disconnect() end
        
        TitleAnimConnection = RunService.Heartbeat:Connect(function()
            -- Cambiar letra cada 0.5 segundos (30 frames @ 60fps)
            TitleAnimIndex = TitleAnimIndex + 1
            TitleColorIndex = TitleColorIndex + 1
            
            if TitleAnimIndex >= 30 then  -- 0.5 segundos
                TitleAnimIndex = 0
                local textIndex = (math.floor(TitleColorIndex / 30) % #CantoLunaLetras) + 1
                TitleLabel.Text = CantoLunaLetras[textIndex]
            end
            
            -- Aplicar color rainbow (cicla continuamente)
            local colorIndex = (TitleColorIndex % (#RainbowColors * 30)) / 30
            ...
            local alpha = colorIndex % 1
            
            TitleLabel.TextColor3 = color1:Lerp(color2, alpha)
        end)
    end
