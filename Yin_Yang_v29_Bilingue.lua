...
local function assignTextAttributes(guiObject, spanishText, englishText)
    if guiObject and guiObject.SetAttribute then
        guiObject:SetAttribute("TextSpanish", spanishText)
        guiObject:SetAttribute("TextEnglish", englishText)
    end
end
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:RefreshLanguage()
        for _, tabData in ipairs(self.Tabs or {}) do
            if tabData.Button then
                for _, child in ipairs(tabData.Button:GetDescendants()) do
                    if child:IsA("TextLabel") or child:IsA("TextButton") or child:IsA("TextBox") then
                        local csp = child:GetAttribute("TextSpanish")
                        local cen = child:GetAttribute("TextEnglish")
                        if csp and cen then
                            child.Text = GetText(csp, cen)
                        end
                    end
                end
            end
