...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// Label de bienvenida
    ChatTab:CreateLabel("💬 Sistema de Mensajería", 14)
    ChatTab:CreateDivider()
    
    --// ScrollingFrame para mostrar mensajes
    ...
    local function RenderMessage(playerName, userId, messageText, timeStamp, isSelf)
        ...
        ChatContainer.CanvasPosition = Vector2.new(0, ChatContainer.CanvasSize.Y)
    end
    
    --// TextBox para escribir mensajes
    ChatTab:CreateLabel("Escribe tu mensaje (máx 500 caracteres)", 11)
    ...
    local function SendMessage()
        local messageText = MessageInput.Text:sub(1, MAX_CHAR)
        
        if messageText == "" or messageText:match("^%s*$") then
            return
        end
    ...
    MessageInput.Changed:Connect(function()
        CharLabel.Text = #MessageInput.Text .. " / 500"
    end)
    
    --// Simular mensajes de otros jugadores (para testing)
    local function SimulateIncomingMessage(playerName, userId, message)
        local timestamp = os.date("%H:%M:%S")
        AddChatMessage(playerName, userId, message, timestamp)
        RenderMessage(playerName, userId, message, timestamp, false)
        playSound(Sounds.Dragon, 0.4)  -- Sonido de notificación
    end
    
    --// Exposer funciones en Window para uso externo
    ...
    function Window:ReceiveMessage(playerName, userId, message)
        SimulateIncomingMessage(playerName, userId, message)
    end
