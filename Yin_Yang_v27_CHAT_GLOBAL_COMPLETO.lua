...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// ══════════════════════════════════════════════════════════════════════════════
    --// PESTAÑA PERMANENTE: CHAT GLOBAL v27 CON INTEGRACIÓN BACKEND REPLIT
    --// ══════════════════════════════════════════════════════════════════════════════
    ...
    function ChatGlobal.Send(playerName, playerId, message)
        ...
        if not ok then
            warn("[ChatGlobal] Error al enviar:", result)
        else
            print("[ChatGlobal] Mensaje enviado: " .. message)
        end
    end
        ...
        task.spawn(function()
            while true do
                ...
                if ok then
                    ...
                    if success and data.messages then
                        local msgs = data.messages
                        local toFire = {}
                        
                        for i = #msgs, 1, -1 do
                            local msg = msgs[i]
                            if not knownIds[msg.id] then
                    ...
                    warn("[ChatGlobal] Error obteniendo mensajes:", result)
                end
    ...
    chatContainer.Size = UDim2.new(1, -10, 0, 300)
    ...
    --// CONECTAR AL BACKEND Y MOSTRAR MENSAJES
    ChatGlobal.Connect(function(msg)
        local msgFrame = Instance.new("Frame")
        msgFrame.Name = "Message_" .. msg.id
        msgFrame.Size = UDim2.new(1, -10, 0, 50)
        ...
        nameLabel.Text = msg.playerName or "Anónimo"
        ...
        -- Actualizar canvas
        chatContainer.CanvasSize = chatContainer.CanvasSize + UDim2.new(0, 0, 0, 55)
    ...
    inputContainer.Position = UDim2.new(0, 5, 0, 360)
    inputContainer.BackgroundColor3 = Color3.fromRGB(35, 35, 40)
    inputContainer.BorderSizePixel = 0
    inputContainer.Parent = AutoTabChat.Page
    
    local inputCorner = Instance.new("UICorner")
    inputCorner.CornerRadius = UDim.new(0, 6)
    inputCorner.Parent = inputContainer
    
    -- TextBox para mensaje
    local messageInput = Instance.new("TextBox")
    messageInput.Name = "MessageInput"
    messageInput.Size = UDim2.new(1, -45, 1, 0)
    ...
    inputCorner2.Parent = messageInput
    ...
    AutoTabChat:CreateLabel("Mensajes sincronizados con todos los jugadores", 10)
