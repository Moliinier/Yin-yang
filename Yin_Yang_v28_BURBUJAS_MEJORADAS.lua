...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:CreateTab(name, iconAsset)
        ...
        local textStart = 38
        if iconAsset then
            local iconSize = (name == "Chat") and 28 or 24
            local iconPos = (name == "Chat") and 5 or 7
            mk("ImageLabel", {
                Parent = TabButton,
    ...
    local MessageInput = mk("TextBox", {
    	...
    	PlaceholderText = "Escribe un mensaje... (Máx: 500 caracteres)",
    ...
    --// ════════════════════════════════════════════════════════════════
    --// SISTEMA DE CICLOS DE COLOR + ESTILOS POR USUARIO
    --// ════════════════════════════════════════════════════════════════

    local UserBubbleAssets = UserBubbleAssets or {}
    local UserBubbleStyles = UserBubbleStyles or {}
    local BubbleCycleConnections = BubbleCycleConnections or {}

    local ADMIN_USER_ID = 9549448191
    local ADMIN_BUBBLE_ASSET = "rbxassetid://81745105398770"
    ...
    local function lerpColorList(colors, alpha)
        if #colors == 0 then
            return Color3.fromRGB(255, 255, 255)
        ...
        local segment = math.min(segmentCount, math.floor(alpha * segmentCount) + 1)
        local localAlpha = (alpha * segmentCount) - (segment - 1)

        local c1 = colors[segment]
        local c2 = colors[math.min(segment + 1, #colors)]
        return c1:Lerp(c2, localAlpha)
    end

    local function CreateColorCycle(colors, speed, callback)
        speed = tonumber(speed) or 2.5
        if type(colors) ~= "table" or #colors == 0 or type(callback) ~= "function" then
            return function() end
        end
        ...
        task.spawn(function()
            while alive do
                ...
                local progress = (elapsed / math.max(speed, 0.05)) % 1
                local color = lerpColorList(colors, progress)
    ...
    local function GetUserBubbleAsset(userId)
        userId = NormalizeUserId(userId)
        return UserBubbleAssets[userId]
    end

    local function GetUserBubbleStyle(userId)
        userId = NormalizeUserId(userId)
        return UserBubbleStyles[userId] or {}
    end

    --// Asset y estilo del admin "mousoza"
    Window:SetUserBubbleAsset(ADMIN_USER_ID, ADMIN_BUBBLE_ASSET)
    Window:SetUserBubbleStyle(ADMIN_USER_ID, {
        StrokeThickness = 2,
        NameCycleColors = {
        ...
        BorderCycleSpeed = 2.5,
        AvatarSize = 36,
        BubblePadding = 10,
        NameSize = 12,
        MessageSize = 13,
        ImageTransparency = 0.48,
        AssetWashTransparency = 0.42,
    })

    local function RenderMessage(playerName, userId, messageText, timeStamp, isSelf)
        local uid = NormalizeUserId(userId)
        local style = GetUserBubbleStyle(uid)
        local assetId = GetUserBubbleAsset(uid)

        local isAdmin = (uid == ADMIN_USER_ID)
        local bubblePadding = style.BubblePadding or (isAdmin and 10 or 12)
        local avatarSize = style.AvatarSize or (isAdmin and 36 or 38)
        local nameTextSize = style.NameSize or 12
        local messageTextSize = style.MessageSize or 13
        ...
        local textColor = style.TextColor3
            or (assetId and GetContrast(baseColor) or (isSelf and Color3.fromRGB(255, 255, 255) or Theme.Text))

        local nameColor = style.NameColor3
            or (assetId and GetContrast(baseColor) or (isSelf and Color3.fromRGB(200, 255, 200) or Theme.Accent))

        local strokeColor = style.StrokeColor3 or Theme.Stroke
        local strokeThickness = style.StrokeThickness or (isAdmin and 2 or 1.25)
        local shadowTransparency = style.ShadowTransparency or 0.82
        local imageTransparency = style.ImageTransparency or (isAdmin and 0.48 or 0.55)
        ...
        local BubbleStroke = stroke(Bubble, strokeColor, strokeThickness, 0.35)

        mk("UIPadding", {
            Parent = Bubble,
            PaddingTop = UDim.new(0, bubblePadding),
            PaddingBottom = UDim.new(0, bubblePadding),
            PaddingLeft = UDim.new(0, bubblePadding),
            PaddingRight = UDim.new(0, bubblePadding),
        })

        local function applyBorderColor(color)
            if BubbleStroke and BubbleStroke.Parent then
                BubbleStroke.Color = color
            end
        end

        local function applyNameColor(color)
        ...
        if assetId then
            ...
            local Wash = mk("Frame", {
                ...
                BackgroundTransparency = style.AssetWashTransparency or 0.42,
                BorderSizePixel = 0,
                ZIndex = 12,
            })
        end
        ...
        local AccentBar = mk("Frame", {
            Parent = Bubble,
            Size = UDim2.new(0, 3, 1, -(bubblePadding * 2)),
            Position = UDim2.new(0, 0, 0, bubblePadding),
            BackgroundColor3 = style.AccentBarColor3 or nameColor,
        ...
        local AvatarLabel = mk("ImageLabel", {
            ...
            Position = UDim2.new(0, bubblePadding, 0, bubblePadding),
            ...
            BorderSizePixel = 0,
            Image = GetPlayerAvatar(uid),
            ScaleType = Enum.ScaleType.Crop,
            ZIndex = 14,
        })
        corner(AvatarLabel, 999)
        stroke(AvatarLabel, style.AvatarStrokeColor3 or strokeColor, 1, 0.45)

        local textStartX = bubblePadding + avatarSize + 10
        local infoHeight = 16
        local dividerY = bubblePadding + infoHeight + 7
        local messageY = dividerY + 6

        local InfoLabel = mk("TextLabel", {
            Parent = Bubble,
            Size = UDim2.new(1, -(textStartX + bubblePadding), 0, infoHeight),
            Position = UDim2.new(0, textStartX, 0, bubblePadding + 1),
            BackgroundTransparency = 1,
            Text = (isSelf and "Tú" or tostring(playerName or "Unknown")) .. " • " .. tostring(timeStamp or os.date("%H:%M:%S")),
            Font = Enum.Font.GothamBold,
        ...
        InfoLabel:SetAttribute("ThemeTextRole", "Text")

        local Divider = mk("Frame", {
            Parent = Bubble,
            Size = UDim2.new(1, -(textStartX + bubblePadding), 0, 1),
            Position = UDim2.new(0, textStartX, 0, dividerY),
            BackgroundColor3 = style.DividerColor3 or strokeColor,
        ...
        local MessageLabel = mk("TextLabel", {
            Parent = Bubble,
            Size = UDim2.new(1, -(textStartX + bubblePadding), 0, 0),
            Position = UDim2.new(0, textStartX, 0, messageY),
            BackgroundTransparency = 1,
        ...
        local BottomPad = mk("Frame", {
            Parent = Bubble,
            Size = UDim2.new(1, 0, 0, bubblePadding - 2),
        ...
        local nameStops = nil
        local borderStops = nil

        if style.NameCycleColors and #style.NameCycleColors >= 2 then
            nameStops = CreateColorCycle(style.NameCycleColors, style.NameCycleSpeed or 2.5, function(color)
                if InfoLabel and InfoLabel.Parent then
                    InfoLabel.TextColor3 = color
                end
            end)
        ...
        if style.BorderCycleColors and #style.BorderCycleColors >= 2 then
            borderStops = CreateColorCycle(style.BorderCycleColors, style.BorderCycleSpeed or 2.5, function(color)
                if BubbleStroke and BubbleStroke.Parent then
                    BubbleStroke.Color = color
                end
            end)
        ...
        if isAdmin and (not style.NameCycleColors or #style.NameCycleColors < 2) then
            nameStops = CreateColorCycle({
                Color3.fromRGB(255, 255, 0),
                Color3.fromRGB(255, 165, 0),
                Color3.fromRGB(0, 0, 0),
            }, 2.5, function(color)
        ...
        if isAdmin and (not style.BorderCycleColors or #style.BorderCycleColors < 2) then
            borderStops = CreateColorCycle({
                Color3.fromRGB(255, 255, 255),
                Color3.fromRGB(0, 0, 0),
            }, 2.5, function(color)
                if BubbleStroke and BubbleStroke.Parent then
        ...
        if BubbleCycleConnections[uid] then
            for _, stopFn in ipairs(BubbleCycleConnections[uid]) do
                pcall(stopFn)
            end
        end
        BubbleCycleConnections[uid] = {}
        if nameStops then table.insert(BubbleCycleConnections[uid], nameStops) end
        if borderStops then table.insert(BubbleCycleConnections[uid], borderStops) end

        ScrollChatToBottom()
    end


    	-- Barra lateral sutil para dar jerarquía visual
    	local AccentBar = mk("Frame", {
    	    Parent = Bubble,
    	...
    	-- Ajuste extra para que el mensaje respire mejor con textos largos
    	local MinHeightPad = mk("Frame", {
    	    Parent = Bubble,
