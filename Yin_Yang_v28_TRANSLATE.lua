...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// callback(translatedText, errorMessage) — solo uno de los dos viene con valor
    local function TranslateMessage(text, callback)
        task.spawn(function()
            local body = HttpService:JSONEncode({
                text = tostring(text),
                auto = true, -- el backend detecta si es español o inglés
            })
    ...
    local function RenderMessage(playerName, userId, messageText, timeStamp, isSelf)
    	...
    	local function ShowTranslation(translatedText)
    	    ...
    	    isTranslated = true
    	    TranslateBtn.Text = GetText("Ver original", "Show original")
    	    ScrollChatToBottom()
    	end
    	...
    	TranslateBtn.MouseButton1Click:Connect(function()
    	    ...
    	    TranslateMessage(messageText, function(translated, err)
