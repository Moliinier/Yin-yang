--[[
    ✅ SCRIPT EVADE CON ESP + YIN YANG v23 - ULTRA FIXED
    - Mejor manejo de errores
    - Validación robusta de librería
...
local function httpGet(url, retries)
    ...
    for attempt = 1, retries do
        ...
        if ok and res and res ~= "" then
            print("✅ Descargado (HttpGet, intento " .. attempt .. ")")
            return res
        end
        ...
        if ok and res and res ~= "" then
            print("✅ Descargado (HttpService, intento " .. attempt .. ")")
            return res
        end

        -- Método 3: http_request / syn.request
        if http_request then
            ok, res = pcall(function() 
                return http_request({Url = url, Method = "GET"}).Body 
            end)
            if ok and res then
                print("✅ Descargado (http_request, intento " .. attempt .. ")")
                return res
            end
        end
        if syn and syn.request then
            ...
            if ok and res then
                print("✅ Descargado (syn.request, intento " .. attempt .. ")")
                return res
            end
        end
        if request then
            ...
            if ok and res then
                print("✅ Descargado (request, intento " .. attempt .. ")")
                return res
            end
        end

        print("⚠️  Intento " .. attempt .. " falló, reintentando...")
        task.wait(0.5)
    end
...
if _G.YinYang and _G.YinYang.CreateWindow then
    ...
    print("📥 YinYang no está cargado, descargando desde GitHub...")
    
    local url = "https://raw.githubusercontent.com/Moliinier/Yin-yang/refs/heads/main/Yin_Yang_v23_OPTIMIZADO.lua"
    print("📍 URL: " .. url)
    
    local code = httpGet(url, 3)
    
    if not code or code == "" then
        warn("❌ CRÍTICO: No se pudo descargar la librería desde GitHub")
        warn("   Posibles razones:")
        warn("   1. Sin conexión a internet")
        warn("   2. Puerto HTTP bloqueado")
        warn("   3. Executor sin permisos HTTP")
        warn("   4. GitHub servidor caído")
        return
    end

    print("✅ Librería descargada (" .. #code .. " bytes)")
    ...
    if not func then
        warn("❌ Error compilando librería: " .. tostring(compileErr))
        return
    end

    local execOk, execErr = pcall(func)
    if not execOk then
        warn("❌ Error ejecutando librería: " .. tostring(execErr))
        return
    end

    if not _G.YinYang or not _G.YinYang.CreateWindow then
        warn("❌ YinYang no se inicializó correctamente en _G")
        print("_G.YinYang = " .. tostring(_G.YinYang))
        return
    end

    YinYang = _G.YinYang
    loadSuccess = true
    print("✅ YinYang compilado y ejecutado exitosamente")
end

if not loadSuccess then
    warn("❌ FALLO CRÍTICO: No se pudo cargar YinYang")
    return
end

print("═══════════════════════════════════════════════════════════")
print("✅ LIBRERÍA CARGADA - Continuando con ESP...")
print("═══════════════════════════════════════════════════════════")
...
if not hasDrawing then
    warn("⚠️  Drawing API no disponible - ShowBoxes desactivado")
end

--// ════════════════════════════════════════════════════════════
--// 4. CREAR UI CON YIN YANG
--// ════════════════════════════════════════════════════════════
...
local createOk, createErr = pcall(function()
    UI = YinYang:CreateWindow("EVADE ESP", "Dark")
end)

if not createOk or not UI then
    warn("❌ Error creando ventana: " .. tostring(createErr))
    print("Detalles: YinYang = " .. tostring(YinYang))
    print("CreateWindow = " .. tostring(YinYang.CreateWindow))
    return
end
...
ESPTab:CreateButton("📌 Visualización de jugadores", function() end)
...
ESPTab:CreateButton("⚙️ Configuración", function() end)
ESPTab:CreateButton("🎯 Distancia máxima: " .. ESPConfig.Distance .. "m", function() end)
...
InfoTab:CreateButton("📋 Información del Script", function() end)
InfoTab:CreateButton("🔧 Script: Evade ESP v1.0 ULTRA FIXED", function() end)
InfoTab:CreateButton("📚 Librería: Yin Yang v23", function() end)
InfoTab:CreateButton("✨ Features: ESP, Nombres, Distancia, Salud", function() end)
...
print("═══════════════════════════════════════════════════════════")
print("✅ EVADE ESP CON YIN YANG v23 CARGADO - ULTRA FIXED")
print("═══════════════════════════════════════════════════════════")
