...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// FUNCIÓN AUXILIAR PARA OBTENER ASSET DINÁMICAMENTE
    local function GetVideoAsset(fileName)
        -- Verifica si la función del ejecutor existe
        if getcustomasset and isfile and isfile(fileName) then
            print("📂 Archivo encontrado: " .. fileName)
            return getcustomasset(fileName)
        end
        print("❌ getcustomasset no disponible o archivo no existe: " .. fileName)
        return nil
    end
    
    --// SOLO GUARDAMOS EL NOMBRE DEL ARCHIVO, NO EL RBXASSET HARDCODEADO
    local ThemeVideoAssets = {
        ["YinYangNewV1"] = "YinYangVideos/video_YinYangNewV1.webm",
    }
    ...
    function Window:SetTheme(themeName)
        ...
        if themeName == "YinYangNewV1" then
            ...
            if CurrentVideoFrame and CurrentVideoFrame.Parent then
                ...
                CurrentVideoFrame.ZIndex = 6
                print("✅ VideoFrame hecho visible")
                
                --// OBTENER ASSET DINÁMICAMENTE
                local fileName = ThemeVideoAssets["YinYangNewV1"]
                local videoAsset = GetVideoAsset(fileName)
                
                if videoAsset then
                    CurrentVideoFrame.Video = videoAsset
                    print("✅ Video asignado a VideoFrame")
                    print("   URL generada: " .. tostring(videoAsset))
                    
                    --// SISTEMA ROBUSTO DE CARGA - ESPERAR A IsLoaded
                    if not CurrentVideoFrame.IsLoaded then
                        print("⏳ Esperando a que el video cargue...")
                        local loadTimeout = 5
                        local timeWaited = 0
                        
                        while not CurrentVideoFrame.IsLoaded and timeWaited < loadTimeout do
                            task.wait(0.1)
                            timeWaited = timeWaited + 0.1
                        end
                        
                        if timeWaited >= loadTimeout then
                            print("⚠ Timeout esperando carga (5s)")
                        end
                    end
                    
                    --// INICIAR REPRODUCCIÓN SI CARGÓ
                    if CurrentVideoFrame.IsLoaded then
                        print("▶ Video cargado exitosamente. Iniciando reproducción...")
                        CurrentVideoFrame:Play()
                        print("   Playing: " .. tostring(CurrentVideoFrame.Playing))
                        print("   TimePosition: " .. tostring(CurrentVideoFrame.TimePosition))
                    else
                        warn("❌ Error: El video no pudo cargar después de 5 segundos")
                        ActivarFallbackImagen()
                    end
                else
                    warn("❌ No se pudo generar asset de video. ¿Archivo en workspace?")
                    ActivarFallbackImagen()
                end
            else
    ...
    --// FUNCIÓN DE FALLBACK PARA CUANDO EL VIDEO NO CARGUE
    local function ActivarFallbackImagen()
        print("🔄 Activando imagen de fallback...")
        if CurrentVideoFrame then
            CurrentVideoFrame.Visible = false
            CurrentVideoFrame:Pause()
        end
        
        -- Mostrar BackgroundArt nuevamente
        if BackgroundArt then
            BackgroundArt.Visible = true
            BackgroundArt.ZIndex = 5
            print("✅ Fallback imagen activada")
        end
    end
