...
local ThemePalettes = {
    ...
    --// SUKUNA V1: Tema oscuro del Rey de las Maldiciones - Negro profundo con rojo
    SukunaV1 = {
        Background = Color3.fromRGB(15, 10, 12),         -- Negro profundo
        Secondary = Color3.fromRGB(35, 20, 25),          -- Negro con toque rojo
        AccentOff = Color3.fromRGB(55, 30, 40),          -- Rojo muy oscuro
        Text = Color3.fromRGB(220, 220, 220),            -- Blanco puro
        TextDim = Color3.fromRGB(150, 80, 100),          -- Rojo oscuro
        Stroke = Color3.fromRGB(180, 60, 80),            -- Rojo profundo
        Accent = Color3.fromRGB(220, 30, 60),            -- Rojo puro
        ToggleOn = Color3.fromRGB(255, 50, 80),          -- Rojo vibrante
    },
}
...
local ThemeBackgroundImages = {
    ...
    ShylfieV1 = "rbxassetid://103506986694652",  -- 👼 Angel mejorado
    SukunaV1 = "rbxassetid://85949954769240",  -- 👹 Rey de las maldiciones
}
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:SetTheme(themeName)
        ...
        elseif themeName == "SukunaV1" then
            self:SetTextEffect("SukunaWhiteBlack")
            print("👹 Tema Sukuna V1 activado + Efecto Blanco-Negro")
        elseif themeName == "ShylfieV1" then
            self:SetTextEffect("ShylfieYellowWhite")
            print("👼 Tema Shylfie V1 activado + Efecto Amarillo-Blanco")
        else
    ...
    function Window:SetTextEffect(mode)
        ...
        elseif mode == "SukunaWhiteBlack" then
            -- 👹 EFECTO ESPECIAL PARA SUKUNA V1: Blanco 2s → Negro 1s (ciclo 3s)
            textEffectConnection = track(RunService.Heartbeat:Connect(function(dt)
                elapsed = elapsed + dt
                local cycle = (elapsed * 0.333) % 1  -- Un ciclo cada 3 segundos
                
                local color
                if cycle < 0.667 then
                    -- Primeros 2 segundos: BLANCO PURO
                    color = Color3.fromRGB(255, 255, 255)
                else
                    -- Último 1 segundo: NEGRO PURO
        ...
        elseif mode == "ShylfieYellowWhite" then
            -- 👼 EFECTO ESPECIAL PARA SHYLFIE V1: Amarillo ↔ Blanco (lento+rápido a la vez)
            textEffectConnection = track(RunService.Heartbeat:Connect(function(dt)
                elapsed = elapsed + dt
                -- Oscilación: lenta en el primer segundo, luego rápida
                local cycle = (elapsed * 1.2) % 2  -- Ciclo de 2 segundos
                
                local color
                if cycle < 1 then
                    -- Primer segundo (LENTO): Amarillo → Blanco
                    local t = cycle  -- Interpolación lenta
                    color = Color3.fromRGB(
                        255,
                        math.floor(255 * (1 - t * 0.5)),  -- Verde baja lentamente
                        0
                    )
                else
                    -- Segundo segundo (RÁPIDO): Blanco → Amarillo
                    local t = (cycle - 1) * 2  -- 2x más rápido
                    color = Color3.fromRGB(
                        255,
                        math.floor(255 - 255 * t),  -- Verde sube rápidamente
                        0
                    )
                end
...
local DEMO_ACTIVO = false  -- ✅ DESACTIVADA - La librería solo se carga, sin ventana automática
