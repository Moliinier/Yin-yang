...
--// GLOW PARA COMPONENTES GRANDES (Main window, panels)
--// Crea el glow como SIBLING en el padre, NO como hijo del frame
local function addGlowLayer(parent, thickness, transparency, color, cornerRadius)
    local container = parent.Parent  -- SIBLING: va en el mismo padre
    ...
    layer.Position = parent.Position
    layer.AnchorPoint = parent.AnchorPoint
    layer.ZIndex = parent.ZIndex - 1  -- Detrás del frame
...
--// GLOW PARA COMPONENTES PEQUEÑOS (FloatingToggle, botones)
--// Solo anima el UIStroke existente, sin capas extra
local function addGlowStrokeOnly(frame, accentColor)
    local existingStroke = frame:FindFirstChildOfClass("UIStroke")
    if not existingStroke then return nil end
    ...
    -- Gradiente en el stroke existente
    ...
    return grad, existingStroke
end

local function buildAnimatedBorder(frame, accentColor, cornerRadius)
    local key = tostring(frame)
    stopGlowTweens(key)

    -- Eliminar GlowLayers anteriores si existen como siblings
    if frame.Parent then
        for _, child in ipairs(frame.Parent:GetChildren()) do
            if child.Name == "GlowLayer" and child ~= frame then
                child:Destroy()
            end
        end
    end
    ...
    -- Determinar si es componente pequeño (FloatingToggle ≤ 210px alto)
    local isSmall = (frame.AbsoluteSize.Y <= 60 or frame.Size == UDim2.fromOffset(200, 40))

    if isSmall then
        -- ✅ COMPONENTE PEQUEÑO: solo animar UIStroke existente
        local grad, existingStroke = addGlowStrokeOnly(frame, accentColor)
        if not grad or not existingStroke then return end
        ...
        -- ✅ COMPONENTE GRANDE: glow como SIBLING detrás del frame
        local _, outerStroke = addGlowLayer(frame, 12, 0.72, accentColor, cr)
        local _, innerStroke = addGlowLayer(frame, 3, 0.30, accentLight, cr)

        if not outerStroke or not innerStroke then return end
        ...
        local pulseTween = TweenService:Create(
            outerStroke,
            TweenInfo.new(1.6, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
            { Transparency = 0.45, Thickness = 16 }
        )
        pulseTween:Play()
    ...
    function Window:CreateTab(nameSpanish, nameEnglishOrIcon, iconAsset)
        ...
        function Tab:CreateFloatingToggle(textSpanish, textEnglishOrDefault, defaultOrCallback, callback)
            ...
            local function createFloatingWindow()
                ...
                --// Borde del tema con efecto de brillo animado
                stroke(FloatingWindow, Theme.Accent, 2, 0.5)
                buildAnimatedBorder(FloatingWindow, Theme.Accent, UDim.new(1, 0))
    ...
    function Window:SetTheme(themeName)
        ...
        for _, floatData in ipairs(Window.FloatingToggles or {}) do
            if floatData.Window and floatData.Window.Parent then
                buildAnimatedBorder(floatData.Window, Theme.Accent, UDim.new(1, 0))
            end
        end
    end
