...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    local BoomboxActive = false

    local BoomboxPill = mk("Frame", {
        Size = UDim2.new(0, 140, 0, 26),
        Position = UDim2.new(0.5, -70, 0.5, -13),
        BackgroundColor3 = Theme.Secondary,
        BackgroundTransparency = 0.2,
        ZIndex = 10,
    }, TopBar)
    BoomboxPill:SetAttribute("ThemeRole", "Secondary")
    corner(BoomboxPill, 999)
    stroke(BoomboxPill, Theme.Accent, 1, 0.4)
    ...
    BoomboxInput:SetAttribute("ThemeTextRole", "Text")
    ...
    local function playBoombox(id)
        ...
        pcall(function()
            ...
            BoomboxSound = sound
            BoomboxActive = true
            print("🎵 Boombox reproduciendo ID: " .. cleanId)
        end)
    end

    BoomboxInput.FocusLost:Connect(function(enterPressed)
        local text = BoomboxInput.Text:gsub("%s+", "")
        if text == "" then
            -- Si está vacío: detener música
            if BoomboxSound then
                pcall(function() BoomboxSound:Stop() BoomboxSound:Destroy() end)
                BoomboxSound = nil
                BoomboxActive = false
                print("🎵 Boombox detenido")
            end
        elseif enterPressed or true then
            playBoombox(text)
        end
    end)
