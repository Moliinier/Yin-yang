...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    AutoTabAjustes:CreateToggle("Más Pequeño", false, function(state)
        if state then
            Window:SetLibraryVersion("Small")
            currentSize = "Small"
            AutoTabAjustes:CreateLabel(Versión Pequeña (Compacta) Activada", 11)
        end
    end)
    
    AutoTabAjustes:CreateToggle("Normal (Original)", true, function(state)
        if state then
            Window:SetLibraryVersion("Normal")
            currentSize = "Normal"
            AutoTabAjustes:CreateLabel(Versión Normal (Original) Activada", 11)
        end
    end)
    
    AutoTabAjustes:CreateToggle("Más Grande", false, function(state)
        if state then
            Window:SetLibraryVersion("Large")
            currentSize = "Large"
            AutoTabAjustes:CreateLabel(Versión Grande Activada", 11)
        end
    end)
