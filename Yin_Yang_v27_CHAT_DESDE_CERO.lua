...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// ══════════════════════════════════════════════════════════════════════════════
    --// PESTAÑA CHAT GLOBAL - CONSTRUIDA DESDE CERO (PROFESIONAL)
    --// ══════════════════════════════════════════════════════════════════════════════
    
    local ChatPage = mk("ScrollingFrame", {
        Size = UDim2.new(1, 0, 1, 0),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ScrollBarThickness = 2,
    ...
    local ChatTab = {Button = mk("TextButton", {
        Size = UDim2.new(1, 0, 0, 32),
        ...
        TextSize = 13,
        ZIndex = 8
    }, TabList), Page = ChatPage}
    
    corner(ChatTab.Button, 6)
    ...
    mk("ImageLabel", {
        ...
        BackgroundTransparency = 1,
        Image = "rbxassetid://135141743431426",
        ZIndex = 9
    })
    
    resetScrollTop(TabList)
    
    table.insert(Window.Tabs, ChatTab)
    ...
    function ChatGlobal.Send(playerName, playerId, message)
        task.spawn(function()
            ...
            pcall(function()
                HttpService:PostAsync(BACKEND_URL .. "/api/chat/send", body, Enum.HttpContentType.ApplicationJson)
            end)
        end)
    end
                ...
                task.spawn(function()
                    ...
                    if ok then
                        ...
                        if success and data and data.messages then
                            ...
                            for _, msg in ipairs(toFire) do task.spawn(callback, msg) end
                        end
                    end
                end)
                task.wait(POLL_RATE)
    ...
    corner(inputBar, 12)
    stroke(inputBar, Color3.fromRGB(100, 100, 120), 1.5)
    ...
    local messageInput = mk("TextBox", {
        ...
        Position = UDim2.new(0, 45, 0.5, -17),
    ...
    mk("UIPadding", {Padding = UDim.new(0, 10)}, messageInput)
    ...
    ChatGlobal.Connect(function(msg)
        task.spawn(function()
            ...
            local newSize = chatContainer.CanvasSize.Y.Offset + 47
    ...
    --// SELECCIONAR CHAT AL HACER CLICK
    ChatTab.Button.MouseButton1Click:Connect(function()
        for _, t in pairs(Window.Tabs) do
            t.Page.Visible = false
        ...
        ChatTab.Button.TextColor3 = Color3.fromRGB(100, 200, 255)
        ChatTab.Button.BackgroundColor3 = Color3.fromRGB(80, 80, 100)
    end)
