...
--// ════════════════════════════════════════════════════════════════
--// EFECTO DE BRILLO ANIMADO - v28 PRO
--// Arquitectura: SIBLING para ventanas grandes
--// Solo UIStroke+UIGradient para controles pequeños
--// ════════════════════════════════════════════════════════════════
...
-- Para ventanas/paneles grandes: crea un SIBLING detrás del frame
local function buildWindowGlow(frame, accentColor, cornerRadius)
    local key = tostring(frame)
    stopGlowTweens(key)

    -- Eliminar glow anterior si existe
    local parent = frame.Parent
    local existingGlow = parent and parent:FindFirstChild("__GlowSibling_" .. frame.Name)
    if existingGlow then existingGlow:Destroy() end

    local cr = cornerRadius or UDim.new(0, 10)

    -- Calcular colores derivados del Accent
    local h, s, v = Color3.toHSV(accentColor)
    local accentLight = Color3.fromHSV(h, math.max(0, s - 0.3), math.min(1, v + 0.3))
    local accentDark  = Color3.fromHSV(h, math.min(1, s + 0.1), math.max(0, v - 0.3))

    -- Crear sibling detrás del frame (mismo padre)
    local glowFrame = Instance.new("Frame")
    glowFrame.Name = "__GlowSibling_" .. frame.Name
    glowFrame.BackgroundTransparency = 1
    glowFrame.Size = frame.Size
    glowFrame.Position = frame.Position
    glowFrame.AnchorPoint = frame.AnchorPoint
    glowFrame.ZIndex = frame.ZIndex - 1
    glowFrame.Parent = parent

    -- Sincronizar posición/tamaño con Main dinámicamente
    ...
    -- UICorner igual al frame
    local gc = Instance.new("UICorner")
    gc.CornerRadius = cr
    gc.Parent = glowFrame

    -- UIStroke con gradiente animado
...
-- Para controles pequeños (toggles flotantes): solo UIStroke + UIGradient
local function applySmallGlow(frame, accentColor, cornerRadius)
    local key = "small_" .. tostring(frame)
    stopGlowTweens(key)

    -- Eliminar stroke existente de glow si hubiera
    local existingGlow = frame:FindFirstChild("__SmallGlowStroke")
    if existingGlow then existingGlow:Destroy() end
    ...
    local glowStroke = Instance.new("UIStroke")
    glowStroke.Name = "__SmallGlowStroke"
    glowStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    -- Efecto de brillo animado: SIBLING en ScreenGui (no hijo de Main)
    local MainGlow = buildWindowGlow(Main, Theme.Accent, UDim.new(0, 10))
    ...
    function Window:CreateTab(nameSpanish, nameEnglishOrIcon, iconAsset)
        ...
        local textStart = 38
        if iconAsset then
            local iconSize = (displayName == "Chat") and 42 or 24
            local iconPos = (displayName == "Chat") and 1 or 7
            mk("ImageLabel", {
                Parent = TabButton,
        ...
        function Tab:CreateFloatingToggle(textSpanish, textEnglishOrDefault, defaultOrCallback, callback)
            ...
            local function createFloatingWindow()
                ...
                --// Borde con glow animado (solo UIStroke+UIGradient, sin capas extra)
                stroke(FloatingWindow, Theme.Accent, 2, 0.4)
                applySmallGlow(FloatingWindow, Theme.Accent, UDim.new(1, 0))
    ...
    function Window:SetTheme(themeName)
        ...
        -- Actualizar glow de la ventana principal (sibling)
        buildWindowGlow(Main, Theme.Accent, UDim.new(0, 10))

        -- Actualizar glow en floating toggles activos (small glow)
        for _, floatData in ipairs(Window.FloatingToggles or {}) do
            if floatData.Window and floatData.Window.Parent then
                applySmallGlow(floatData.Window, Theme.Accent, UDim.new(1, 0))
            end
        end
    end
