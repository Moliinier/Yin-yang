...
--// 🎵 SONIDOS DE CLICK PERSONALIZADOS POR TEMA (v27)
...
--// 🎵 v27: VARIABLE PARA ACTIVAR/DESACTIVAR SONIDOS PERSONALIZADOS
...
--// 🌙 LETRAS DE "CANTO DE LUNA" PARA TÍTULO ANIMADO (v27)
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    --// 🌙 SISTEMA DE TÍTULO ANIMADO CON RAINBOW (v27) - TEMPORALMENTE DESACTIVADO
    ...
    function Window:CreateTab(name, iconAsset)
        ...
        --// 🎚️ FLOATING TOGGLE SIMPLE (v27 - NUEVO DISEÑO ELEGANTE)
        function Tab:CreateFloatingToggleSimple(text, default, callback)
            ...
            local FloatingWindow = mk("Frame", {
                ...
                BackgroundColor3 = Theme.Secondary,
                BackgroundTransparency = 0.6,  -- 🎨 MÁS TRANSPARENTE (0.6 en lugar de 0.3)
                BorderSizePixel = 0,
                ZIndex = 150,
            })
            
            --// ESQUINAS REDONDEADAS (Cápsula)
            corner(FloatingWindow, 999)
            
            --// BORDE ANIMADO CON COLORES GIRANDO (v27 PREMIUM)
            local BorderStroke = stroke(FloatingWindow, Theme.Accent, 2, 0.5)
            FloatingWindow:SetAttribute("ThemeRole", "Secondary")
            
            --// TABLA DE COLORES PARA ANIMACIÓN DE BORDE
            local borderColors = {
            ...
            borderAnimConnection = RunService.Heartbeat:Connect(function()
                ...
                if borderColorIndex > #borderColors then
                    borderColorIndex = 1
                end
                pcall(function()
            ...
            --// ETIQUETA DE TEXTO + INDICADOR OFF/ON
            local TextLabel = mk("TextLabel", {
            ...
            TextLabel:SetAttribute("ThemeTextRole", "Text")
            
            --// INDICADOR OFF/ON (Lado derecho)
            ...
            local pulseSize = UDim2.new(0, 170, 0, 55)
            ...
            FloatingWindow.MouseEnter:Connect(function()
                ...
                TweenService:Create(
                    ...
                    {BackgroundTransparency = 0.4}  -- Más opaco al hover
                ):Play()
            end)
            ...
            FloatingWindow.MouseLeave:Connect(function()
                ...
                TweenService:Create(
                    FloatingWindow,
                    TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out),
                    {BackgroundTransparency = 0.6}  -- Vuelve a translúcido
                ):Play()
            end)
            
            --// EFECTO CLICK Y ACTIVACIÓN
            ClickDetector.MouseButton1Click:Connect(function()
                state = not state
                playSound(Sounds.Click, 0.6)
                
                --// ACTUALIZAR INDICADOR OFF/ON
                ...
                TweenService:Create(
                    ...
                    {Size = originalSize, BackgroundTransparency = isHovering and 0.4 or 0.6}
                ):Play()
            ...
            track(FloatingWindow.InputBegan:Connect(OnInputBegan))
            ...
            table.insert(Window.FloatingToggles, {Window = FloatingWindow, Name = text, Type = "Simple"})
        ...
        function Tab:CreateToggle(text, default, callback)
            ...
            ToggleFrame:SetAttribute("ThemeRole", "Secondary")
            ...
            local ToggleButton = mk("Frame", {
                Parent = ToggleFrame,
                Size = UDim2.new(0, 35, 0, 20),
                Position = UDim2.new(1, -45, 0.5, -10),
                BackgroundColor3 = (default and Theme.ToggleOn) or Theme.AccentOff,
                BorderSizePixel = 0,
            })
            ToggleButton:SetAttribute("ThemeRole", default and "ToggleOn" or "AccentOff")
            corner(ToggleButton, 10)

            local Circle = mk("Frame", {
                Parent = ToggleButton,
                Size = UDim2.new(0, 16, 0, 16),
                Position = UDim2.new(default and 0.5 or 0, default and 9 or 2, 0.5, -8),
            ...
            ToggleButton.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    ...
                    TweenService:Create(
                        Circle,
                        TweenInfo.new(0.2),
                        {Position = UDim2.new(state and 0.5 or 0, state and 9 or 2, 0.5, -8)}
                    ):Play()
        ...
        function Tab:CreateSlider(text, min, max, default, callback)
            ...
            SliderContainer:SetAttribute("ThemeRole", "Secondary")
            corner(SliderContainer, 6)
            ...
            local function UpdateSlider(percentage)
                currentValue = math.clamp(min + (max - min) * percentage, min, max)
            ...
            SliderBackground.InputBegan:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseButton1 then
                    local relX = (input.Position.X - SliderBackground.AbsolutePosition.X) / SliderBackground.AbsoluteSize.X
                    pcall(function() UpdateSlider(math.clamp(relX, 0, 1)) end)
                end
            end)
        ...
        --// 🎛️ CREATE DROPDOWN (NUEVO v27)
        function Tab:CreateDropdown(text, options, default, callback)
            local DropdownFrame = mk("Frame", {
                Parent = TabPage,
                Size = UDim2.new(1, 0, 0, 35),
                BackgroundColor3 = Theme.Secondary,
                BorderSizePixel = 0,
            })
            DropdownFrame:SetAttribute("ThemeRole", "Secondary")
            ...
            local DropdownButton = mk("TextButton", {
                Parent = DropdownFrame,
            ...
            DropdownButton:SetAttribute("ThemeRole", "Background")
            ...
            DropdownButton.MouseButton1Click:Connect(function()
                if isOpen then
                    DropdownButton.Size = UDim2.new(0.55, -10, 0.8, 0)
                    DropdownButton.Text = selectedValue
                    isOpen = false
                else
                    DropdownButton.Size = UDim2.new(0.55, -10, 0, #options * 25 + 10)
                    isOpen = true
                end
            end)
        ...
        --// 📝 CREATE TEXTBOX (NUEVO v27)
        function Tab:CreateTextBox(text, placeholder, callback)
            local TextBoxFrame = mk("Frame", {
                Parent = TabPage,
                Size = UDim2.new(1, 0, 0, 40),
                BackgroundColor3 = Theme.Secondary,
                BorderSizePixel = 0,
            })
            TextBoxFrame:SetAttribute("ThemeRole", "Secondary")
            corner(TextBoxFrame, 6)
            ...
            local InputBox = mk("TextBox", {
                Parent = TextBoxFrame,
                Size = UDim2.new(text == "" and 1 or 0.65, text == "" and -10 or 0, 0.7, 0),
                Position = UDim2.new(text == "" and 0.05 or 0.32, 0, 0.15, 0),
                BackgroundColor3 = Theme.Background,
                BorderSizePixel = 0,
                PlaceholderText = placeholder or "Ingresa texto...",
    ...
    function Window:SetTheme(themeName)
        ...
        CurrentTheme = themeName  -- 🎵 v27: Actualizar variable global
        ...
        --// 💾 v27: GUARDAR CONFIGURACIÓN AUTOMÁTICAMENTE
        SavedConfig.CurrentTheme = themeName
        SaveConfig()

        --// 🎵 v27: SONIDO DE CLICK DINÁMICO POR TEMA
    ...
    --// ════════════════════════════════════════════════════════════════
    --// CREAR AUTOMÁTICAMENTE LAS 3 PESTAÑAS SAGRADAS (v27 MEJORADO)
    --// ════════════════════════════════════════════════════════════════
    
    -- TAB 1: INICIO (Automático)
    local AutoTabInicio = Window:CreateTab("Inicio")
    AutoTabInicio:CreateWelcomeCard()
    AutoTabInicio:CreateDivider()
    AutoTabInicio:CreateServerInfoCard()
    
    -- TAB 2: TEMAS (Automático)
    local AutoTabTemas = Window:CreateTab("Temas")
    ...
    local AutoTabEfectos = Window:CreateTab("Efectos")
...
print("✅ Yin Yang v27 - Floating Toggle Simple 🎚️ - ¡Librería cargada y lista para usar!")
...
local DEMO_ACTIVO = false  -- ❌ DESACTIVADA - Solo librería pura

if DEMO_ACTIVO then
    task.wait(0.5)
    
    print("\n" .. string.rep("=", 60))
    print("INICIANDO DEMO VISUAL DE YIN YANG v27 - LIBRERÍA PROFESIONAL")
    print(string.rep("=", 60))
    
    --// 💾 v27: CARGAR CONFIGURACIÓN GUARDADA AL INICIAR
    ...
    local DemoUI = _G.YinYang:CreateWindow("Yin Yang - DEMO v27", TemaInicial)
    ...
    -- =========================================================
    -- TAB FLOATING TOGGLES (DEMO NUEVA v27)
    -- =========================================================
    local TabFloatingToggles = DemoUI:CreateTab("Floating Toggles")
    
    TabFloatingToggles:CreateLabel("🎚️ Toggles Flotantes Elegantes", 14)
    TabFloatingToggles:CreateDivider()
    TabFloatingToggles:CreateLabel("Presiona los botones de abajo para crear toggles flotantes", 11)
    TabFloatingToggles:CreateDivider()
    
    --// BOTÓN PARA CREAR FLOATING TOGGLE 1
    TabFloatingToggles:CreateButton("➕ Auto Spawn", function()
        print("✅ Creando Floating Toggle: Auto Spawn")
        TabFloatingToggles:CreateFloatingToggleSimple("Auto Spawn", false, function(state)
            if state then
    ...
    --// BOTÓN PARA CREAR FLOATING TOGGLE 2
    TabFloatingToggles:CreateButton("➕ Instant Revive", function()
        print("✅ Creando Floating Toggle: Instant Revive")
        TabFloatingToggles:CreateFloatingToggleSimple("Instant Revive", false, function(state)
            if state then
    ...
    --// BOTÓN PARA CREAR FLOATING TOGGLE 3
    TabFloatingToggles:CreateButton("➕ Auto Carry", function()
        print("✅ Creando Floating Toggle: Auto Carry")
        TabFloatingToggles:CreateFloatingToggleSimple("Auto Carry", false, function(state)
            if state then
    ...
    TabFloatingToggles:CreateDivider()
    TabFloatingToggles:CreateLabel("💡 Tip: Los toggles flotantes son dra ggables. Puedes moverlos por la pantalla.", 10)
    TabFloatingToggles:CreateLabel("✨ Efecto Shimmer automático + Breathing effect + Hover/Click feedback", 10)
    ...
    TabFeatures:CreateFloatingToggleSimple("Aimbot", false, function(state)
        print("Aimbot: " .. (state and "ON" or "OFF"))
    end)
    
    TabFeatures:CreateFloatingToggleSimple("ESP", false, function(state)
        print("ESP: " .. (state and "ON" or "OFF"))
    end)
    
    TabFeatures:CreateFloatingToggleSimple("GodMode", false, function(state)
    ...
    print("Yin Yang v27 - DEMO DESACTIVADA (DEMO_ACTIVO = false)")
