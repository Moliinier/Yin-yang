...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:CreateTab(name, iconAsset)
        ...
        --// SELECTOR DE FUENTES DE TEXTO PERSONALIZADO
        function Tab:CreateFontSelector(label, default, callback)
            ...
            Container:SetAttribute("ThemeRole", "Secondary")
            corner(Container, 6)
            stroke(Container, Color3.fromRGB(0, 0, 0), 1, 0.6)
            ...
            local currentFont = default or "GothamMedium"
            
            local fontButton = mk("TextButton", {
                Parent = Container,
            ...
            fontButton:SetAttribute("ThemeRole", "Text")
            corner(fontButton, 4)

            fontButton.MouseButton1Click:Connect(function()
                local currentIndex = table.find(fonts, currentFont) or 1
                currentFont = fonts[(currentIndex % #fonts) + 1]
                fontButton.Text = "📝 " .. currentFont
                
                if callback then
                    callback(currentFont)
                end
            end)
    ...
    function Window:SetTheme(themeName)
        ...
        if themeName == "CatV1" then
            -- Tema especial del gato: Sonido meow especial (133371725828981) + Efecto rainbow rosa-blanco rápido
            CurrentClickSound = "rbxassetid://133371725828981"  -- 🐱 Meow especial SOLO en Cat V1
    ...
    function Window:SetTextEffect(mode)
        ...
        elseif mode == "RainbowDarkWhite" then
            -- 🌙 EFECTO YIN-YANG: Oscilación lenta entre Negro y Blanco
            -- Ciclo lento de 1 segundo (0.5s por mitad)
            local colorBlack = Color3.fromRGB(0, 0, 0)
            local colorWhite = Color3.fromRGB(255, 255, 255)
            textEffectConnection = track(RunService.Heartbeat:Connect(function(dt)
                elapsed = elapsed + dt
                local alpha = (math.sin(elapsed * 2) + 1) / 2  -- Oscilación suave y lenta
                applyTextColorToAll(colorBlack:Lerp(colorWhite, alpha))
            end))
...
if DEMO_ACTIVO then
    ...
    TabInicio:CreateLabel("✨ 17 Temas personalizados (incl. 🐱 Cat V1)\n📱 Toggles flotantes desprendibles\n🌈 6 Efectos de texto (incl. CatRainbow y RainbowDarkWhite)\n📝 Fuentes de texto personalizables\n🔊 Sonido meow especial en Cat V1", 10)
    ...
    local temas = {
        ...
        "Green", "Orange", "NaranjaV1", "VioletaV1",
    ...
    TabEfectos:CreateDivider()
    TabEfectos:CreateLabel("Efecto Gradiente (Yin-Yang)", 11)

    TabEfectos:CreateButton("🌙 Negro - Blanco (Lento)", function()
        DemoUI:SetTextEffect("RainbowDarkWhite")
        print("🌙 Efecto: Oscilación lenta Negro ↔ Blanco - Efecto Yin-Yang perfecto!")
    end)
