...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:SetTheme(themeName)
        ...
        if themeName == "CatV1" then
            -- Tema especial del gato: Sonido especial meow (133371725828981) + Efecto rainbow rosa-blanco rápido
            CurrentClickSound = "rbxassetid://133371725828981"  -- 🐱 Sonido meow especial SOLO en Cat V1
    ...
    function Window:SetTextEffect(mode)
        ...
        elseif mode == "RainbowDarkWhite" then
            -- 🌙 EFECTO LENTO: Oscilación suave entre Negro y Blanco
            -- Velocidad lenta (0.5 segundos por ciclo)
            local colorBlack = Color3.fromRGB(0, 0, 0)
            local colorWhite = Color3.fromRGB(255, 255, 255)
            textEffectConnection = track(RunService.Heartbeat:Connect(function(dt)
                elapsed = elapsed + dt
                local alpha = (math.sin(elapsed * 2) + 1) / 2  -- Ciclo lento de 0.5 seg
...
if DEMO_ACTIVO then
    ...
    TabEfectos:CreateDivider()
    TabEfectos:CreateLabel("Efecto Gradiente", 11)

    TabEfectos:CreateButton("🌙 Negro - Blanco (Lento)", function()
        DemoUI:SetTextEffect("RainbowDarkWhite")
        print("🌙 Efecto: Oscilación lenta Negro ↔ Blanco - Efecto Yin-Yang!")
    end)
