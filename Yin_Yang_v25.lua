...
--// 💾 SISTEMA DE GUARDADO: Persistencia entre sesiones
local ConfigStore = nil
pcall(function()
    ConfigStore = DataStoreService:GetDataStore("YinYangConfig_v25")
end)

local function saveConfig(configData)
    if not ConfigStore then return end
    pcall(function()
        ConfigStore:SetAsync("PlayerConfig_" .. LocalPlayer.UserId, configData)
    end)
end

local function loadConfig()
    if not ConfigStore then return {theme = "Dark", effect = "Off"} end
    local config = {theme = "Dark", effect = "Off"}
    pcall(function()
        local saved = ConfigStore:GetAsync("PlayerConfig_" .. LocalPlayer.UserId)
        if saved then
            config = saved
        end
    end)
...
--// 🔒 SISTEMA DE SINGLETON: La librería solo se carga UNA VEZ
local LIBRARY_ALREADY_LOADED = _G.YinYangLoaded or false

if LIBRARY_ALREADY_LOADED then
    print("✅ Yin Yang ya está cargado. La librería será reutilizada.")
else
    _G.YinYangLoaded = true
end
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    startTheme = startTheme or config.theme or "Dark"
    ...
    function Window:SetTheme(themeName)
        ...
        if themeName == "CatV1" then
            ...
            CurrentClickSound = "rbxassetid://133371725828981"  -- 🐱 Meow especial
            self:SetTextEffect("CatRainbow")
            print("🐱 Tema Cat V1 activado - Sonido meow especial + Efecto rainbow rosa-blanco")
        else
            -- Otros temas: Sonido normal + DESACTIVAR efecto CatRainbow
            CurrentClickSound = Sounds.Click
            if self.CurrentTextEffect == "CatRainbow" then
                self:SetTextEffect("Off")  -- 🔄 Desactivar CatRainbow al cambiar tema
            end
        end
        ...
        elseif themeName == "LightV1" then
            ...
            -- Otros temas: Sonido de click normal
            CurrentClickSound = Sounds.Click
        ...
        --// 💜 PURPLE CON ANIMACIÓN SATORU: Pulsación de poder sobrenatural
        if themeName == "Purple" then
            if BackgroundArt then
                ...
                animationConnection = RunService.Heartbeat:Connect(function(dt)
                    elapsed = elapsed + dt
                    if BackgroundArt and BackgroundArt.Parent then
                        -- Pulsación suave como poder de Satoru
            ...
            print("💜 Tema Purple Satoru activado - Efecto de poder pulsante")
        end
        
        CurrentTheme = themeName
        
        -- 💾 Guardar configuración
        saveConfig({theme = themeName, effect = "Off"})
    end
...
if DEMO_ACTIVO and not LIBRARY_ALREADY_LOADED then
    task.wait(0.5)
    ...
    local temas = {
        ...
        "CatV1",  -- 🐱 Tema del gato
        "LightV1",  -- ✨ Tema claro
        "Purple"  -- 💜 Tema Purple con animación Satoru
    }
