...
local TranslationRegistry = setmetatable({}, { __mode = "k" })

local function GetText(spanishText, englishText)
    if type(spanishText) == "table" then
        local spec = spanishText
        local spanish = spec.Spanish or spec.spanish or spec.ES or spec.es or spec.TextSpanish or spec.Text or spec.Name or spec.Title or spec[1] or ""
        local english = spec.English or spec.english or spec.EN or spec.en or spec.TextEnglish or spec.Text or spec.Name or spec.Title or spec[2] or spanish
        spanishText, englishText = tostring(spanish), tostring(english)
    else
        spanishText = tostring(spanishText or "")
        if englishText == nil or englishText == "" then
            englishText = spanishText
        end
        englishText = tostring(englishText)
    end
...
local function normalizeTextSpec(spec, fallbackEnglish)
    if type(spec) == "table" then
        local spanish = spec.Spanish or spec.spanish or spec.ES or spec.es or spec.TextSpanish or spec.Text or spec.Name or spec.Title or spec[1] or ""
        local english = spec.English or spec.english or spec.EN or spec.en or spec.TextEnglish or spec.Text or spec.Name or spec.Title or spec[2] or spanish
        return tostring(spanish), tostring(english)
    end

    local spanish = tostring(spec or "")
    local english = fallbackEnglish
    if english == nil or english == "" then
        english = spanish
    end
    return spanish, tostring(english)
end

local function assignTextAttributes(guiObject, spanishText, englishText)
    if not guiObject then
        return nil, nil
    end

    local spanish, english = normalizeTextSpec(spanishText, englishText)
    TranslationRegistry[guiObject] = {
        Spanish = spanish,
        English = english,
    }
...
local function getTranslatedText(guiObject)
    local data = TranslationRegistry[guiObject]
    if not data then
        return nil, nil
    end
    return data.Spanish, data.English
end
...
local function createStatTile(grid, labelSpanish, labelEnglish)
    ...
    assignTextAttributes(TileLabel, labelSpanish, labelEnglish)
...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    corner(TopBar, 10)
    local TitleDivider = mk("Frame", {Size = UDim2.new(1, 0, 0, 1), Position = UDim2.new(0, 0, 1, -1), BackgroundColor3 = Theme.Stroke, ZIndex = 7}, TopBar)
    ...
    function Window:SetLanguage(newLanguage)
        if newLanguage ~= "es" and newLanguage ~= "en" then
            warn("Idioma no válido. Usa 'es' o 'en'")
            return false
        end

        if LanguageSystem.CurrentLanguage == newLanguage then
            return true
        end

        LanguageSystem.CurrentLanguage = newLanguage
        LanguageSystem.Config.Language = newLanguage
        SavedConfig.Language = newLanguage
        SaveLanguageConfig()
        SaveConfig()
    ...
    function Window:RefreshLanguage()
        for obj, data in pairs(TranslationRegistry) do
            if obj and obj.Parent then
                pcall(function()
                    if obj:IsA("TextLabel") or obj:IsA("TextButton") or obj:IsA("TextBox") then
                        obj.Text = GetText(data.Spanish, data.English)
                    end
                end)
            else
                TranslationRegistry[obj] = nil
            end
        end
    end
    ...
    function Window:CreateTab(nameSpanish, nameEnglishOrIcon, iconAsset)
        ...
        if type(nameSpanish) == "table" then
            local spec = nameSpanish
            nameSpanish, nameEnglish = normalizeTextSpec(spec)
            iconAsset = spec.Icon or spec.icon or spec.Asset or spec.AssetId or spec.IconAsset or iconAsset
        elseif type(nameEnglishOrIcon) == "table" then
            local spec = nameEnglishOrIcon
            nameSpanish, nameEnglish = normalizeTextSpec(spec, nameSpanish)
            iconAsset = spec.Icon or spec.icon or spec.Asset or spec.AssetId or spec.IconAsset or iconAsset
        elseif type(nameEnglishOrIcon) == "string" and nameEnglishOrIcon:find("rbxassetid", 1, true) then
            --// Código antiguo: CreateTab(name, iconAsset)
            iconAsset = nameEnglishOrIcon
            nameEnglish = nameSpanish
        elseif type(nameEnglishOrIcon) == "string" then
        ...
        assignTextAttributes(TabButton, nameSpanish, nameEnglish)
        corner(TabButton, 6)
        ...
        assignTextAttributes(TabNameLabel, nameSpanish, nameEnglish)
        ...
        function Tab:CreateFloatingToggle(textSpanish, textEnglishOrDefault, defaultOrCallback, callback)
            --// COMPATIBILIDAD:
            --// 1) Antiguo: CreateFloatingToggle(text, default, callback)
            --// 2) Nuevo bilingüe: CreateFloatingToggle(textSpanish, textEnglish, default, callback)
            --// 3) Nuevo con tabla: CreateFloatingToggle({Spanish=..., English=..., Default=..., Callback=...})
            local textEnglish = textSpanish
            local default, cb

            if type(textSpanish) == "table" then
                local spec = textSpanish
                textSpanish, textEnglish = normalizeTextSpec(spec)
                default = spec.Default
                    or spec.DefaultState
                    or spec.State
                    or spec.Value
                    or false
                cb = spec.Callback or spec.Function or spec.OnChanged
            elseif type(textEnglishOrDefault) == "string" then
                textEnglish = textEnglishOrDefault
                default = defaultOrCallback
                cb = callback
            elseif type(textEnglishOrDefault) == "table" then
                local spec = textEnglishOrDefault
                textSpanish, textEnglish = normalizeTextSpec(spec, textSpanish)
                default = spec.Default
                    or spec.DefaultState
                    or spec.State
                    or spec.Value
                    or defaultOrCallback
                    or false
                cb = spec.Callback or spec.Function or spec.OnChanged or callback
            else
            ...
            assignTextAttributes(HolderLabel, textSpanish, textEnglish)
            ...
            local function createFloatingWindow()
                ...
                assignTextAttributes(FloatLabel, textSpanish, textEnglish)
        ...
        function Tab:CreateToggle(textSpanish, textEnglishOrDefault, defaultOrCallback, callback)
            --// COMPATIBILIDAD:
            --// 1) Antiguo: CreateToggle(text, default, callback)
            --// 2) Nuevo bilingüe: CreateToggle(textSpanish, textEnglish, default, callback)
            --// 3) Nuevo con tabla: CreateToggle({Spanish=..., English=..., Default=..., Callback=...})
            local textEnglish = textSpanish
            local default, cb
            ...
            assignTextAttributes(LabelTxt, textSpanish, textEnglish)
        ...
        function Tab:CreateSlider(text, min, max, default, callback)
            local textEnglish = text

            if type(text) == "table" then
                local spec = text
                text, textEnglish = normalizeTextSpec(spec)
                min = spec.Min or spec.Minimum or min
                max = spec.Max or spec.Maximum or max
                default = spec.Default or spec.Value or default
                callback = spec.Callback or spec.Function or spec.OnChanged or callback
            end
            ...
            assignTextAttributes(LabelTxt, text, textEnglish)
        ...
        function Tab:CreateButton(textSpanish, textEnglishOrCallback, callbackOrIcon, iconAsset)
            --// COMPATIBILIDAD:
            --// 1) Antiguo: CreateButton(text, callback, iconAsset)
            --// 2) Nuevo bilingüe: CreateButton(textSpanish, textEnglish, callback, iconAsset)
            --// 3) Nuevo con tabla: CreateButton({Spanish=..., English=..., Callback=..., Icon=...})
            local textEnglish = textSpanish
            local callback, icon
            ...
            if type(textSpanish) == "table" then
                local spec = textSpanish
                textSpanish, textEnglish = normalizeTextSpec(spec)
                callback = spec.Callback or spec.Function or spec.OnClick or textEnglishOrCallback
                icon = spec.Icon or spec.icon or spec.Asset or spec.AssetId or spec.IconAsset or callbackOrIcon or iconAsset
            elseif type(textEnglishOrCallback) == "string" then
            ...
            if iconAsset then
                ...
                assignTextAttributes(BtnLabel, textSpanish, textEnglish)
            else
                --// Sin icono: el texto vive directo en el TextButton
                                assignTextAttributes(Btn, textSpanish, textEnglish)
            end
        ...
        function Tab:CreateLabel(textSpanish, textEnglishOrSize, fontSize)
            ...
            assignTextAttributes(Label, textSpanish, textEnglish)
            resetScrollTop(TabPage)
            return Label
        end
        ...
        function Tab:CreateWelcomeCard()
            ...
            assignTextAttributes(WelcomeLabel, "Bienvenido,", "Welcome,")
        ...
        function Tab:CreateServerInfoCard()
            ...
            assignTextAttributes(ServerLabel, "Servidor", "Server")
    ...
    --// ════════════════════════════════════════════════════════════════
    --// UPDATE LOOP — Cambio instantáneo de idioma (v29)
    --// ════════════════════════════════════════════════════════════════
...
print(" Yin Yang v29 BILINGÜE - ¡Librería cargada y lista para usar!")
...
if DEMO_ACTIVO then
    ...
    print("\n" .. string.rep("=", 60))
    print("INICIANDO DEMO VISUAL DE YIN YANG v29 - LIBRERÍA PROFESIONAL")
    print(string.rep("=", 60))
    ...
    local DemoUI = _G.YinYang:CreateWindow("Yin Yang - DEMO v29", TemaInicial)
