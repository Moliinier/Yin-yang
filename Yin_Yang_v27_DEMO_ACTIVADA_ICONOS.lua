...
function YinYang:CreateWindow(title_text, startTheme)
    ...
    function Window:SetTheme(themeName)
        ...
        --// SONIDO DE CLICK DINÁMICO POR TEMA
        ...
        --// v27 NUEVA CARACTERÍSTICA: CAMBIAR ICONOS A VERSIÓN GATITO EN CatV1
        if themeName == "CatV1" then
            -- Cambiar iconos de pestañas a versión gatito
            for _, tabBtn in ipairs(TabList:GetChildren()) do
                if tabBtn:IsA("TextButton") then
                    -- Buscar ImageLabel dentro del botón
                    local img = tabBtn:FindFirstChild("ImageLabel")
                    if img then
                        if tabBtn.Text:find("Inicio") or string.find(tostring(tabBtn), "Inicio") then
                            img.Image = "rbxassetid://73765130079426"  -- Gatito Inicio
                        elseif tabBtn.Text:find("Temas") or string.find(tostring(tabBtn), "Temas") then
                            img.Image = "rbxassetid://112948010230169"  -- Gatito Temas
                        elseif tabBtn.Text:find("Efectos") or string.find(tostring(tabBtn), "Efectos") then
                            img.Image = "rbxassetid://70512676896175"   -- Gatito Efectos
                        elseif tabBtn.Text:find("Ajustes") or string.find(tostring(tabBtn), "Ajustes") then
                            img.Image = "rbxassetid://131379643038273"  -- Gatito Ajustes
                        end
                    end
                end
            end
            print("🐱 Tema CatV1 activado - Iconos gatito aplicados")
        else
            -- Restaurar iconos originales cuando se sale de CatV1
            for _, tabBtn in ipairs(TabList:GetChildren()) do
                if tabBtn:IsA("TextButton") then
                    local img = tabBtn:FindFirstChild("ImageLabel")
                    if img then
                        if tabBtn.Text:find("Inicio") or string.find(tostring(tabBtn), "Inicio") then
                            img.Image = "rbxassetid://124987849953130"  -- Casa original
                        elseif tabBtn.Text:find("Temas") or string.find(tostring(tabBtn), "Temas") then
                            img.Image = "rbxassetid://84419345138935"   -- Manzana original
                        elseif tabBtn.Text:find("Efectos") or string.find(tostring(tabBtn), "Efectos") then
                            img.Image = "rbxassetid://114693810646148"  -- Rayo original
                        elseif tabBtn.Text:find("Ajustes") or string.find(tostring(tabBtn), "Ajustes") then
                            img.Image = "rbxassetid://86797720103644"   -- Ajustes original
                        end
                    end
                end
            end
    ...
    AutoTabAjustes:CreateLabel("⚙️ Configuración", 14)
    ...
    AutoTabAjustes:CreateToggle("🔒 Freeze Icono", false, function(state)
        ...
        if state then
            ...
            AutoTabAjustes:CreateLabel("❌ Icono desbloqueado (Se puede mover)", 11)
        end
    end)
